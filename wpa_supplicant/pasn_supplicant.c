                                  ...
                                  u16 comeback_after)
{
    if (comeback)
    {
        ...
        if (comeback_txt)
        {
            wpa_snprintf_hex(comeback_txt, buflen, wpabuf_head(comeback), comeback_len);

            wpa_msg(wpa_s, MSG_INFO, PASN_AUTH_STATUS MACSTR " akmp=%s, status=%u comeback_after=%u comeback=%s",
                    MAC2STR(bssid), wpa_key_mgmt_txt(akmp, WPA_PROTO_RSN), status, comeback_after, comeback_txt);
    ...
    wpa_msg(wpa_s, MSG_INFO, PASN_AUTH_STATUS MACSTR " akmp=%s, status=%u", MAC2STR(bssid),
            wpa_key_mgmt_txt(akmp, WPA_PROTO_RSN), status);
}
...
static struct wpabuf *wpas_pasn_wd_sae_commit(struct wpa_supplicant *wpa_s)
{
    ...
    ret = sae_prepare_commit_pt(&pasn->sae, pasn->ssid->pt, wpa_s->own_addr, pasn->bssid, NULL, NULL);
    if (ret)
    {
...
static int wpas_pasn_wd_sae_rx(struct wpa_supplicant *wpa_s, struct wpabuf *wd)
{
    ...
    wpa_printf(MSG_DEBUG, "PASN: SAE: commit: alg=%u, seq=%u, status=%u", alg, seq, status);

    if (alg != WLAN_AUTH_SAE || seq != 1 || status != WLAN_STATUS_SAE_HASH_TO_ELEMENT)
    {
    ...
    wpa_printf(MSG_DEBUG, "PASN: SAE confirm: alg=%u, seq=%u, status=%u", alg, seq, status);
...
static int wpas_pasn_sae_setup_pt(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, int group)
{
...
static struct wpabuf *wpas_pasn_fils_build_auth(struct wpa_supplicant *wpa_s)
{
    ...
    if (!erp_msg)
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: ERP EAP-Initiate/Re-auth unavailable");
        return NULL;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "PASN: FILS: Nonce", pasn->fils.nonce, FILS_NONCE_LEN);

    wpa_hexdump(MSG_DEBUG, "PASN: FILS: Session", pasn->fils.session, FILS_SESSION_LEN);
     ...
     */
    ret = fils_pmkid_erp(pasn->akmp, wpabuf_head(erp_msg), wpabuf_len(erp_msg), pasn->fils.erp_pmkid);
    if (ret)
    {
...
static struct wpabuf *wpas_pasn_wd_fils_auth(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_printf(MSG_DEBUG, "PASN: FILS: wrapped data - completed=%u", pasn->fils.completed);
    ...
    if (!(fils_info & BIT(9)))
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS auth without PFS not supported");
        return NULL;
    }
...
static int wpas_pasn_wd_fils_rx(struct wpa_supplicant *wpa_s, struct wpabuf *wd)
{
    ...
    wpa_hexdump(MSG_DEBUG, "PASN: FILS: Authentication frame len=%zu", data, buf_len);
    ...
    wpa_printf(MSG_DEBUG, "PASN: FILS: commit: alg=%u, seq=%u, status=%u", alg, seq, status);

    if (alg != WLAN_AUTH_FILS_SK || seq != 2 || status != WLAN_STATUS_SUCCESS)
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: Dropping peer authentication");
        return -1;
    }
    ...
    if (!elems.rsn_ie || !elems.fils_nonce || !elems.fils_nonce || !elems.wrapped_data)
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: Missing IEs");
        return -1;
    }

    ret = wpa_parse_wpa_ie(elems.rsn_ie - 2, elems.rsn_ie_len + 2, &rsne_data);
    if (ret)
    {
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: Not expecting PMKID in RSNE");
        return -1;
    }

    wpa_hexdump(MSG_DEBUG, "PASN: FILS: ANonce", elems.fils_nonce, FILS_NONCE_LEN);
    os_memcpy(anonce, elems.fils_nonce, FILS_NONCE_LEN);

    wpa_hexdump(MSG_DEBUG, "PASN: FILS: FILS Session", elems.fils_session, FILS_SESSION_LEN);

    if (os_memcmp(pasn->fils.session, elems.fils_session, FILS_SESSION_LEN))
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: Session mismatch");
        return -1;
    }

    fils_wd = ieee802_11_defrag(&elems, WLAN_EID_EXTENSION, WLAN_EID_EXT_WRAPPED_DATA);

    if (!fils_wd)
    {
        wpa_printf(MSG_DEBUG, "PASN: FILS: Failed getting wrapped data");
        return -1;
    }

    eapol_sm_process_erp_finish(wpa_s->eapol, wpabuf_head(fils_wd), wpabuf_len(fils_wd));
    ...
    ret = fils_rmsk_to_pmk(pasn->akmp, rmsk, rmsk_len, pasn->fils.nonce, anonce, NULL, 0, pasn->pmk, &pasn->pmk_len);
    ...
    wpa_hexdump(MSG_DEBUG, "PASN: FILS: PMKID", pasn->fils.erp_pmkid, PMKID_LEN);

    wpa_printf(MSG_DEBUG, "PASN: FILS: ERP processing succeeded");

    wpa_pasn_pmksa_cache_add(wpa_s->wpa, pasn->pmk, pasn->pmk_len, pasn->fils.erp_pmkid, pasn->bssid, pasn->akmp);
...
static struct wpabuf *wpas_pasn_get_wrapped_data(struct wpa_supplicant *wpa_s)
{
    ...
    switch (pasn->akmp)
    {
...
#endif /* CONFIG_SAE */
            wpa_printf(MSG_ERROR, "PASN: SAE: Cannot derive wrapped data");
            return NULL;
        ...
        case WPA_KEY_MGMT_FT_IEEE8021X_SHA384:
            ...
            return NULL;
        default:
            wpa_printf(MSG_ERROR, "PASN: TODO: Wrapped data for akmp=0x%x", pasn->akmp);
            return NULL;
    }
}
...
static struct wpabuf *wpas_pasn_build_auth_1(struct wpa_supplicant *wpa_s, const struct wpabuf *comeback)
{
    struct wpas_pasn *pasn = &wpa_s->pasn;
    ...
    wpa_pasn_build_auth_header(buf, pasn->bssid, wpa_s->own_addr, pasn->bssid, pasn->trans_seq + 1,
                               WLAN_STATUS_SUCCESS);

    pmkid = NULL;
    if (wpa_key_mgmt_ft(pasn->akmp))
    {
        ret = wpa_pasn_ft_derive_pmk_r1(wpa_s->wpa, pasn->akmp, pasn->bssid, pasn->pmk_r1, &pasn->pmk_r1_len,
                                        pasn->pmk_r1_name);
        if (ret)
        {
            wpa_printf(MSG_DEBUG, "PASN: FT: Failed to derive keys");
            goto fail;
        }
    ...
    else if (wrapped_data != WPA_PASN_WRAPPED_DATA_NO)
    {
        ...
        pmksa = wpa_sm_pmksa_cache_get(wpa_s->wpa, pasn->bssid, NULL, NULL, pasn->akmp);
        if (pmksa)
            pmkid = pmksa->pmkid;
    ...
    wpa_pasn_add_parameter_ie(buf, pasn->group, wrapped_data, pubkey, true, comeback, -1);
    ...
    ret = pasn_auth_frame_hash(pasn->akmp, pasn->cipher, wpabuf_head_u8(buf) + IEEE80211_HDRLEN,
                               wpabuf_len(buf) - IEEE80211_HDRLEN, pasn->hash);
    if (ret)
    {
...
static struct wpabuf *wpas_pasn_build_auth_3(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_pasn_add_parameter_ie(buf, pasn->group, wrapped_data, NULL, false, NULL, -1);
    ...
    ret = pasn_mic(pasn->ptk.kck, pasn->akmp, pasn->cipher, wpa_s->own_addr, pasn->bssid, pasn->hash, mic_len * 2, data,
                   data_len, mic);
    if (ret)
    {
                             ...
                             struct wpabuf *wrapped_data)
{
    ...
    if (pasn->akmp == WPA_KEY_MGMT_PASN)
    {
        ...
        os_memcpy(pasn->pmk, pasn_default_pmk, sizeof(pasn_default_pmk));
        return 0;
    }
    ...
    if (rsn_data->num_pmkid)
    {
        ...
        pmksa = wpa_sm_pmksa_cache_get(wpa_s->wpa, pasn->bssid, rsn_data->pmkid, NULL, pasn->akmp);
        if (pmksa)
        {
    ...
    if (pasn->akmp == WPA_KEY_MGMT_SAE)
    {
        ...
        if (ret)
        {
            wpa_printf(MSG_DEBUG, "PASN: Failed processing SAE wrapped data");
            pasn->status = WLAN_STATUS_UNSPECIFIED_FAILURE;
            return -1;
        }
        ...
        wpa_pasn_pmksa_cache_add(wpa_s->wpa, pasn->pmk, pasn->pmk_len, pasn->sae.pmkid, pasn->bssid, pasn->akmp);
        return 0;
    }
#endif /* CONFIG_SAE */

#ifdef CONFIG_FILS
    if (pasn->akmp == WPA_KEY_MGMT_FILS_SHA256 || pasn->akmp == WPA_KEY_MGMT_FILS_SHA384)
    {
        int ret;
        ...
        if (ret)
        {
            wpa_printf(MSG_DEBUG, "PASN: Failed processing FILS wrapped data");
            pasn->status = WLAN_STATUS_UNSPECIFIED_FAILURE;
            return -1;
        }
...
static int wpas_pasn_start(struct wpa_supplicant *wpa_s,
                           const u8 *bssid,
                           int akmp,
                           int cipher,
                           u16 group,
                           int freq,
                           const u8 *beacon_rsne,
                           u8 beacon_rsne_len,
                           const u8 *beacon_rsnxe,
                           u8 beacon_rsnxe_len,
                           int network_id,
                           struct wpabuf *comeback)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: Reject unsuitable group %u", group);
        return -1;
    }
    ...
    switch (akmp)
    {
        ...
        case WPA_KEY_MGMT_SAE:
            if (!ssid)
            {
                wpa_printf(MSG_DEBUG, "PASN: No network profile found for SAE");
                return -1;
            }

            if (!ieee802_11_rsnx_capab(beacon_rsnxe, WLAN_RSNX_CAPAB_SAE_H2E))
            {
    ...
    pasn->beacon_rsne_rsnxe = wpabuf_alloc(beacon_rsne_len + beacon_rsnxe_len);
    ...
    if (beacon_rsnxe && beacon_rsnxe_len)
        wpabuf_put_data(pasn->beacon_rsne_rsnxe, beacon_rsnxe, beacon_rsnxe_len);
    ...
    if (wpa_s->conf->force_kdk_derivation || (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS2_SEC_LTF &&
    ...
    wpa_printf(MSG_DEBUG, "PASN: Init: " MACSTR " akmp=0x%x, cipher=0x%x, group=%u", MAC2STR(pasn->bssid), pasn->akmp,
    ...
    ret = wpa_drv_send_mlme(wpa_s, wpabuf_head(frame), wpabuf_len(frame), 0, pasn->freq, 1000);
...
static struct wpa_bss *wpas_pasn_allowed(struct wpa_supplicant *wpa_s, const u8 *bssid, int akmp, int cipher)
{
    struct wpa_bss *bss;
    const u8 *rsne;
    struct wpa_ie_data rsne_data;
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: Not doing authentication with current BSS");
        return NULL;
    }
    ...
    if (!(rsne_data.key_mgmt & akmp) || !(rsne_data.pairwise_cipher & cipher))
    {
        wpa_printf(MSG_DEBUG, "PASN: AP does not support requested AKMP or cipher");
        return NULL;
    }
...
static void wpas_pasn_auth_start_cb(struct wpa_radio_work *work, int deinit)
{
     ...
     * established.
     */
    bss = wpas_pasn_allowed(wpa_s, awork->bssid, awork->akmp, awork->cipher);
    if (!bss)
    {
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "PASN: Failed to start PASN authentication");
        goto fail;
    }
                         ...
                         size_t comeback_len)
{
    ...
    wpa_printf(MSG_DEBUG, "PASN: Start: " MACSTR " akmp=0x%x, cipher=0x%x", MAC2STR(bssid), akmp, cipher);
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: Driver does not support offchannel TX");
        return -1;
    }

    if (radio_work_pending(wpa_s, "pasn-start-auth"))
    {
        wpa_printf(MSG_DEBUG, "PASN: send_auth: Work is already pending");
        return -1;
    }
    ...
    if (radio_add_work(wpa_s, bss->freq, "pasn-start-auth", 1, wpas_pasn_auth_start_cb, awork) < 0)
    {
        wpas_pasn_free_auth_work(awork);
        return -1;
    }
...
void wpas_pasn_auth_stop(struct wpa_supplicant *wpa_s)
{
    ...
    wpas_pasn_auth_status(wpa_s, pasn->bssid, pasn->akmp, pasn->cipher, pasn->status, pasn->comeback,
                          pasn->comeback_after);
                                     ...
                                     struct wpa_pasn_params_data *params)
{
    ...
    return wpas_pasn_auth_start(wpa_s, bssid, akmp, cipher, group, network_id, params->comeback, params->comeback_len);
}
...
int wpas_pasn_auth_rx(struct wpa_supplicant *wpa_s, const struct ieee80211_mgmt *mgmt, size_t len)
{
    ...
    u16 status;
    int ret, inc_y;
    u16 fc = host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_AUTH << 4));

    if (!wpa_s->pasn_auth_work || !mgmt || len < offsetof(struct ieee80211_mgmt, u.auth.variable))
        return -2;
    ...
    if (os_memcmp(mgmt->da, wpa_s->own_addr, ETH_ALEN) != 0 || os_memcmp(mgmt->sa, pasn->bssid, ETH_ALEN) != 0 ||
    ...
    if (mgmt->u.auth.auth_transaction != host_to_le16(pasn->trans_seq + 1))
    {
        wpa_printf(MSG_DEBUG, "PASN: RX: Invalid transaction sequence: (%u != %u)",
                   le_to_host16(mgmt->u.auth.auth_transaction), pasn->trans_seq + 1);
        return -1;
    }

    status = le_to_host16(mgmt->u.auth.status_code);

    if (status != WLAN_STATUS_SUCCESS && status != WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY)
    {
        wpa_printf(MSG_DEBUG, "PASN: Authentication rejected - status=%u", status);
        pasn->status = status;
        wpas_pasn_auth_stop(wpa_s);
        return -1;
    }

    if (ieee802_11_parse_elems(mgmt->u.auth.variable, len - offsetof(struct ieee80211_mgmt, u.auth.variable), &elems,
                               0) == ParseFailed)
    {
        wpa_printf(MSG_DEBUG, "PASN: Failed parsing Authentication frame");
        goto fail;
    }
    ...
    if (status == WLAN_STATUS_SUCCESS)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "PASN: Invalid MIC. Expecting len=%u", mic_len);
            goto fail;
        }
        else
        {
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: Missing PASN Parameters IE");
        goto fail;
    }

    ret = wpa_pasn_parse_parameter_ie(elems.pasn_params - 3, elems.pasn_params_len + 3, true, &pasn_params);
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "PASN: Failed validation PASN of Parameters IE");
        goto fail;
    }

    if (status == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY)
    {
        wpa_printf(MSG_DEBUG, "PASN: Authentication temporarily rejected");

        if (pasn_params.comeback && pasn_params.comeback_len)
        {
            wpa_printf(MSG_DEBUG, "PASN: Comeback token available. After=%u", pasn_params.after);

            if (!pasn_params.after)
                return wpas_pasn_immediate_retry(wpa_s, pasn, &pasn_params);

            pasn->comeback = wpabuf_alloc_copy(pasn_params.comeback, pasn_params.comeback_len);
            if (pasn->comeback)
    ...
    ret = wpa_parse_wpa_ie(elems.rsn_ie - 2, elems.rsn_ie_len + 2, &rsn_data);
    if (ret)
    {
    ...
    if (pasn->akmp != rsn_data.key_mgmt || pasn->cipher != rsn_data.pairwise_cipher)
    {
    ...
    {
        wpa_printf(MSG_DEBUG, "PASN: Invalid first octet in pubkey=0x%x", pasn_params.pubkey[0]);
        goto fail;
    }

    secret = crypto_ecdh_set_peerkey(pasn->ecdh, inc_y, pasn_params.pubkey + 1, pasn_params.pubkey_len - 1);
    ...
    {
        wrapped_data = ieee802_11_defrag(&elems, WLAN_EID_EXTENSION, WLAN_EID_EXT_WRAPPED_DATA);
    ...
    ret = pasn_pmk_to_ptk(pasn->pmk, pasn->pmk_len, wpa_s->own_addr, pasn->bssid, wpabuf_head(secret),
                          wpabuf_len(secret), &pasn->ptk, pasn->akmp, pasn->cipher, pasn->kdk_len);
    if (ret)
    {
    ...
    /* Verify the MIC */
    ret = pasn_mic(pasn->ptk.kck, pasn->akmp, pasn->cipher, pasn->bssid, wpa_s->own_addr,
                   wpabuf_head(pasn->beacon_rsne_rsnxe), wpabuf_len(pasn->beacon_rsne_rsnxe), (u8 *)&mgmt->u.auth,
                   len - offsetof(struct ieee80211_mgmt, u.auth), out_mic);
    ...
    ret = wpa_drv_send_mlme(wpa_s, wpabuf_head(frame), wpabuf_len(frame), 0, pasn->freq, 100);
    wpabuf_free(frame);
    if (ret)
    {
    ...
    ptksa_cache_add(wpa_s->ptksa, pasn->bssid, pasn->cipher, dot11RSNAConfigPMKLifetime, &pasn->ptk);
...

{
    ...
    if (!wpa_s->pasn_auth_work)
    {
        wpa_printf(MSG_DEBUG, "PASN: auth_tx_status: no work in progress");
        return -1;
    }

    if (!mgmt || data_len < offsetof(struct ieee80211_mgmt, u.auth.variable))
        return -1;
    ...
    /* Not our frame; do nothing */
    if (os_memcmp(mgmt->da, pasn->bssid, ETH_ALEN) || os_memcmp(mgmt->sa, wpa_s->own_addr, ETH_ALEN) ||
    ...
    {
        wpa_printf(MSG_ERROR, "PASN: Invalid transaction sequence: (%u != %u)", pasn->trans_seq,
                   le_to_host16(mgmt->u.auth.auth_transaction));
        return 0;
    }

    wpa_printf(MSG_ERROR, "PASN: auth with trans_seq=%u, acked=%u", pasn->trans_seq, acked);
    ...
    if (pasn->trans_seq == 3)
    {
        wpa_printf(MSG_DEBUG, "PASN: auth complete with: " MACSTR, MAC2STR(pasn->bssid));
        /*
...
int wpas_pasn_deauthenticate(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    if (os_memcmp(wpa_s->bssid, bssid, ETH_ALEN) == 0)
    {
        wpa_printf(MSG_DEBUG, "PASN: Cannot deauthenticate from current BSS");
        return -1;
    }

    wpa_printf(MSG_DEBUG, "PASN: deauth: Flushing all PTKSA entries for " MACSTR, MAC2STR(bssid));
    ...
    deauth = wpabuf_put(buf, offsetof(struct ieee80211_mgmt, u.deauth.variable));

    deauth->frame_control = host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_DEAUTH << 4));

    os_memcpy(deauth->da, bssid, ETH_ALEN);
    os_memcpy(deauth->sa, wpa_s->own_addr, ETH_ALEN);
    os_memcpy(deauth->bssid, bssid, ETH_ALEN);
    deauth->u.deauth.reason_code = host_to_le16(WLAN_REASON_PREV_AUTH_NOT_VALID);
     ...
     */
    ret = wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, bss->freq, 0);
