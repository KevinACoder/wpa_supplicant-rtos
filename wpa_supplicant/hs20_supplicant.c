...
void hs20_configure_frame_filters(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Not configuring frame filtering - BSS " MACSTR " is not a Hotspot 2.0 network",
                   MAC2STR(bssid));
        return;
    }
    ...
    if (!ie || ie[1] < 5)
        wpa_printf(MSG_DEBUG, "Not configuring frame filtering - Can't extract DGAF bit");
    ...
    {
        wpa_printf(MSG_DEBUG, "Not configuring frame filtering - Can't extract Proxy ARP bit");
        return;
    }
...
{
    if (!ssid->roaming_consortium_selection || !ssid->roaming_consortium_selection_len)
        return;
    ...
    wpabuf_put_data(buf, ssid->roaming_consortium_selection, ssid->roaming_consortium_selection_len);
}
...
static struct wpabuf *hs20_build_anqp_req(u32 stypes, const u8 *payload, size_t payload_len)
{
    struct wpabuf *buf;
...
static struct icon_entry *hs20_find_icon(struct wpa_supplicant *wpa_s, const u8 *bssid, const char *file_name)
{
    struct icon_entry *icon;

    dl_list_for_each(icon, &wpa_s->icon_head, struct icon_entry, list)
    {
        if (os_memcmp(icon->bssid, bssid, ETH_ALEN) == 0 && os_strcmp(icon->file_name, file_name) == 0 && icon->image)
            return icon;
    }
                  ...
                  size_t buf_len)
{
    ...
    wpa_printf(MSG_DEBUG, "HS20: Get icon " MACSTR " %s @ %u +%u (%u)", MAC2STR(bssid), file_name, (unsigned int)offset,
    ...
    b64 = wpa_base64_encode(&icon->image[offset], size, &b64_size);
...
{
    wpa_printf(MSG_DEBUG, "HS20: Free stored icon from " MACSTR " dialog_token=%u file_name=%s image_len=%u",
               MAC2STR(icon->bssid), icon->dialog_token, icon->file_name ? icon->file_name : "N/A",
...
int hs20_del_icon(struct wpa_supplicant *wpa_s, const u8 *bssid, const char *file_name)
{
    ...
    if (!bssid)
        ...
        wpa_printf(MSG_DEBUG, "HS20: Delete all stored icons for " MACSTR, MAC2STR(bssid));
    else
        wpa_printf(MSG_DEBUG, "HS20: Delete stored icons for " MACSTR " file name %s", MAC2STR(bssid), file_name);
...
static void hs20_set_osu_access_permission(const char *osu_dir, const char *fname)
{
    struct stat statbuf;
...
static void hs20_remove_duplicate_icons(struct wpa_supplicant *wpa_s, struct icon_entry *new_icon)
{
    struct icon_entry *icon, *tmp;
...
static int hs20_process_icon_binary_file(
    struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *pos, size_t slen, u8 dialog_token)
{
    char fname[256];
    int png;
    FILE *f;
    ...
    dl_list_for_each(icon, &wpa_s->icon_head, struct icon_entry, list)
    {
        if (icon->dialog_token == dialog_token && !icon->image && os_memcmp(icon->bssid, sa, ETH_ALEN) == 0)
        {
            ...
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ICON MACSTR " %s %u", MAC2STR(sa), icon->file_name,
                    (unsigned int)icon->image_len);
            return 0;
        }
    }

    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Icon Binary File", MAC2STR(sa));
    ...
    if (wpa_s->osu_icon_id == 0)
        wpa_s->osu_icon_id++;
    snprintf(fname, sizeof(fname), "%s/osu-icon-%u.%s", wpa_s->conf->osu_dir, wpa_s->osu_icon_id, png ? "png" : "icon");
    f = fopen(fname, "wb");
    if (f == NULL)
        return -1;
    ...
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, const u8 *sa, const u8 *data, size_t slen, u8 dialog_token)
{
    ...
    switch (subtype)
    {
        case HS20_STYPE_CAPABILITY_LIST:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " HS Capability List", MAC2STR(sa));
        ...
        case HS20_STYPE_OPERATOR_FRIENDLY_NAME:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Operator Friendly Name", MAC2STR(sa));
        ...
        case HS20_STYPE_WAN_METRICS:
            ...
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " WAN Metrics %02x:%u:%u:%u:%u:%u", MAC2STR(sa), pos[0],
                    WPA_GET_LE32(pos + 1), WPA_GET_LE32(pos + 5), pos[9], pos[10], WPA_GET_LE16(pos + 11));
            if (anqp)
            {
        ...
        case HS20_STYPE_CONNECTION_CAPABILITY:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Connection Capability", MAC2STR(sa));
        ...
        case HS20_STYPE_OPERATING_CLASS:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Operating Class", MAC2STR(sa));
        ...
        case HS20_STYPE_OSU_PROVIDERS_LIST:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " OSU Providers list", MAC2STR(sa));
        ...
        case HS20_STYPE_OPERATOR_ICON_METADATA:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Operator Icon Metadata", MAC2STR(sa));
            wpa_hexdump(MSG_DEBUG, "Operator Icon Metadata", pos, slen);
            if (anqp)
            {
                wpabuf_free(anqp->hs20_operator_icon_metadata);
                anqp->hs20_operator_icon_metadata = wpabuf_alloc_copy(pos, slen);
            }
            break;
        case HS20_STYPE_OSU_PROVIDERS_NAI_LIST:
            wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " OSU Providers NAI List", MAC2STR(sa));
            if (anqp)
            {
                wpabuf_free(anqp->hs20_osu_providers_nai_list);
                anqp->hs20_osu_providers_nai_list = wpabuf_alloc_copy(pos, slen);
            }
            break;
        default:
...
static void hs20_osu_fetch_done(struct wpa_supplicant *wpa_s)
{
    ...
    for (i = 0; i < wpa_s->osu_prov_count; i++)
    {
        ...
        {
            fprintf(f, "osu_ssid2=%s\n", wpa_ssid_txt(osu->osu_ssid2, osu->osu_ssid2_len));
        }
        if (osu->osu_nai[0])
            fprintf(f, "osu_nai=%s\n", osu->osu_nai);
        if (osu->osu_nai2[0])
        ...
        {
            fprintf(f, "friendly_name=%s:%s\n", osu->friendly_name[j].lang, osu->friendly_name[j].text);
        }
        for (j = 0; j < osu->serv_desc_count; j++)
        {
            fprintf(f, "desc=%s:%s\n", osu->serv_desc[j].lang, osu->serv_desc[j].text);
        }
        for (j = 0; j < osu->icon_count; j++)
        {
            struct osu_icon *icon = &osu->icon[j];
            ...
            fprintf(f, "icon=%u:%u:%u:%s:%s:%s\n", icon->id, icon->width, icon->height, icon->lang, icon->icon_type,
                    icon->filename);
        }
    }
    fclose(f);
...
void hs20_next_osu_icon(struct wpa_supplicant *wpa_s)
{
    ...
    for (i = 0; i < wpa_s->osu_prov_count; i++)
    {
        ...
        for (j = 0; j < osu->icon_count; j++)
        {
            ...
            if (hs20_anqp_send_req(wpa_s, osu->bssid, BIT(HS20_STYPE_ICON_REQUEST), (u8 *)icon->filename,
                                   os_strlen(icon->filename), 0) < 0)
            {
                icon->failed = 1;
                continue;
            }
...
static void hs20_osu_add_prov(struct wpa_supplicant *wpa_s,
                              struct wpa_bss *bss,
                              const u8 *osu_ssid,
                              u8 osu_ssid_len,
                              const u8 *osu_ssid2,
                              ...
                              size_t len)
{
    ...
    prov = os_realloc_array(wpa_s->osu_prov, wpa_s->osu_prov_count + 1, sizeof(*prov));
    if (prov == NULL)
        return;
    wpa_s->osu_prov = prov;
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "HS 2.0: Not enough room for OSU "
    ...
    while (pos - pos2 >= 4 && prov->friendly_name_count < OSU_MAX_ITEMS)
    {
        ...
        if (slen < 3)
        {
            wpa_printf(MSG_DEBUG, "Invalid OSU Friendly Name (no room for language)");
            break;
        }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "HS 2.0: Not enough room for OSU Server "
                   "URI");
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "HS 2.0: Not enough room for OSU Method "
                   "list length");
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "HS 2.0: Not enough room for Icons "
                   "Available Length");
        return;
    }
    len2 = WPA_GET_LE16(pos);
    ...
    wpa_printf(MSG_DEBUG, "HS 2.0: Added OSU Provider through " MACSTR, MAC2STR(bss->bssid));
...
void hs20_osu_icon_fetch(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list)
    {
        ...
        if (ie && wpa_parse_wpa_ie(ie, 2 + ie[1], &data) == 0 && (data.key_mgmt & WPA_KEY_MGMT_OSEN))
        {
            osu_ssid2     = bss->ssid;
        ...
        wpa_printf(MSG_DEBUG, "HS 2.0: Parsing OSU Providers list from " MACSTR, MAC2STR(bss->bssid));
        ...
        while (end - pos > 2 && num_providers > 0)
        {
            ...
            if (len > (unsigned int)(end - pos))
                break;
            hs20_osu_add_prov(wpa_s, bss, osu_ssid, osu_ssid_len, osu_ssid2, osu_ssid2_len, pos, len);
            pos += len;
        }
        ...
        if (!prov_anqp)
            continue;
        wpa_printf(MSG_DEBUG, "HS 2.0: Parsing OSU Providers NAI List from " MACSTR, MAC2STR(bss->bssid));
        wpa_hexdump_buf(MSG_DEBUG, "HS 2.0: OSU Providers NAI List", prov_anqp);
        pos           = wpabuf_head(prov_anqp);
        end           = pos + wpabuf_len(prov_anqp);
        num_providers = 0;
        while (end - pos > 0)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "HS 2.0: Ignore unexpected OSU Provider NAI List entries");
                break;
            }
            os_memcpy(wpa_s->osu_prov[num_providers].osu_nai2, pos, len);
            pos += len;
            num_providers++;
        }
    }
...
static void hs20_osu_scan_res_handler(struct wpa_supplicant *wpa_s, struct wpa_scan_results *scan_res)
{
...
{
    if (url)
        wpa_msg(wpa_s, MSG_INFO, HS20_SUBSCRIPTION_REMEDIATION "%u %s", osu_method, url);
    else
...
void hs20_rx_deauth_imminent_notice(struct wpa_supplicant *wpa_s, u8 code, u16 reauth_delay, const char *url)
{
    ...
    wpa_msg(wpa_s, MSG_INFO, HS20_DEAUTH_IMMINENT_NOTICE "%u %u %s", code, reauth_delay, url);
    ...
    if (code == HS20_DEAUTH_REASON_CODE_BSS)
    {
        ...
        if (wpa_s->current_ssid)
        {
            ...
            if (now.sec + reauth_delay <= wpa_s->current_ssid->disabled_until.sec)
                return;
            wpa_printf(MSG_DEBUG, "HS 2.0: Disable network for %u seconds (BSS)", reauth_delay);
    ...
    if (code == HS20_DEAUTH_REASON_CODE_ESS && wpa_s->current_ssid)
    {
        ...
        if (now.sec + reauth_delay <= wpa_s->current_ssid->disabled_until.sec)
            return;
        wpa_printf(MSG_DEBUG, "HS 2.0: Disable network for %u seconds", reauth_delay);
        wpa_s->current_ssid->disabled_until.sec = now.sec + reauth_delay;
    }
}
...
void hs20_rx_t_c_acceptance(struct wpa_supplicant *wpa_s, const char *url)
{
    if (!wpa_sm_pmf_enabled(wpa_s->wpa))
    {
        wpa_printf(MSG_DEBUG, "HS 2.0: Ignore Terms and Conditions Acceptance since PMF was not enabled");
        return;
    }
