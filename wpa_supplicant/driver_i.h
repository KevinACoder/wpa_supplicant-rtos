...
static inline int wpas_drv_nan_publish(struct wpa_supplicant *wpa_s, const u8 *addr,
...
static inline int wpas_drv_nan_cancel_publish(struct wpa_supplicant *wpa_s, int publish_id)
{
...
static inline int wpas_drv_nan_update_publish(struct wpa_supplicant *wpa_s, int publish_id,
			    const struct wpabuf *ssi)
{
    if (!wpa_s->driver->nan_update_publish)
        return 0;

    return wpa_s->driver->nan_update_publish(wpa_s->drv_priv, publish_id, ssi);
}

static inline int wpas_drv_nan_subscribe(struct wpa_supplicant *wpa_s, const u8 *addr,
...
static inline int wpas_drv_nan_cancel_subscribe(struct wpa_supplicant *wpa_s, int subscribe_id)
{
    if (!wpa_s->driver->nan_cancel_subscribe)
        return 0;

    return wpa_s->driver->nan_cancel_subscribe(wpa_s->drv_priv, subscribe_id);
}
