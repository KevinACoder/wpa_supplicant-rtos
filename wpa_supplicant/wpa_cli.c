...
static int wpa_cli_show_event(const char *event)
{
     ...
     */
    if (str_starts(start, WPA_EVENT_BSS_ADDED) || str_starts(start, WPA_EVENT_BSS_REMOVED))
        return 0;
...
{
    char *buf             = NULL;
    unsigned int buf_size = CMD_BUF_LEN;
    size_t len;
    int ret;

    buf = os_zalloc(sizeof(u8) * buf_size);
    if (!buf)
    {
    ...
    if (ifname_prefix)
    {
        os_snprintf(buf, buf_size, "IFNAME=%s %s", ifname_prefix, cmd);
        buf[buf_size - 1] = '\0';
        cmd               = buf;
    }
    len = buf_size - 1;
...
static int wpa_ctrl_command(struct wpa_ctrl *ctrl, const char *cmd)
{
    ...
    repl_buf = wpa_supplicant_ctrl_iface_process(wpa_s, (char *)cmd, (size_t *)&len);
...
static int wpa_cli_cmd(struct wpa_ctrl *ctrl, const char *cmd, int min_args, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid %s command - at least %d argument%s "
            "required.\n",
...
static int wpa_cli_cmd_pmksa_flush(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_mesh_cmd_pmksa_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_PMKSA_GET", 1, argc, argv);
}

static int wpa_cli_mesh_cmd_pmksa_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_reassociate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_preauthenticate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_scan_interval(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "SCAN_INTERVAL", 1, argc, argv);
}

static int wpa_cli_cmd_bss_expire_age(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "BSS_EXPIRE_AGE", 1, argc, argv);
}

static int wpa_cli_cmd_bss_expire_count(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_wps_check_pin(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_CHECK_PIN", 1, argc, argv);
}

static int wpa_cli_cmd_wps_cancel(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_wps_nfc_config_token(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_NFC_CONFIG_TOKEN", 1, argc, argv);
}

static int wpa_cli_cmd_wps_nfc_token(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_NFC_TOKEN", 1, argc, argv);
}

static int wpa_cli_cmd_wps_nfc_tag_read(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    int ret;
    char *buf;
    size_t buflen;
    ...
    {
        PRINTF(
            "Invalid 'wps_nfc_tag_read' command - one argument "
            "is required.\n");
        return -1;
    }
...
static int wpa_cli_cmd_nfc_get_handover_req(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "NFC_GET_HANDOVER_REQ", 2, argc, argv);
}

static int wpa_cli_cmd_nfc_get_handover_sel(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "NFC_GET_HANDOVER_SEL", 2, argc, argv);
}

static int wpa_cli_cmd_nfc_report_handover(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_wps_reg(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    else if (argc == 5 || argc == 6)
    {
        ...
        res = os_snprintf(cmd, sizeof(cmd), "WPS_REG %s %s %s %s %s %s", argv[0], argv[1], ssid_hex, argv[3], argv[4],
                          key_hex);
    }
    else
    {
        PRINTF(
            ...
            "- AP PIN\n");
        PRINTF(
            "Alternatively, six arguments can be used to "
            "reconfigure the AP:\n"
            "- BSSID of the target AP\n"
            "- AP PIN\n"
            "- new SSID\n"
...
static int wpa_cli_cmd_wps_ap_pin(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_AP_PIN", 1, argc, argv);
}

static int wpa_cli_cmd_wps_er_start(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_ER_START", 0, argc, argv);
}

static int wpa_cli_cmd_wps_er_stop(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "WPS_ER_STOP");
}

static int wpa_cli_cmd_wps_er_pin(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc < 2)
    {
        PRINTF(
            "Invalid WPS_ER_PIN command: need at least two "
            "arguments:\n"
...
static int wpa_cli_cmd_wps_er_pbc(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WPS_ER_PBC", 1, argc, argv);
}

static int wpa_cli_cmd_wps_er_learn(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc != 2)
    {
        PRINTF(
...
static int wpa_cli_cmd_wps_er_set_config(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc != 2)
    {
        PRINTF(
            "Invalid WPS_ER_SET_CONFIG command: need two "
            "arguments:\n"
            "- UUID: specify which AP to use\n"
...
static int wpa_cli_cmd_wps_er_config(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[256];
    int res;
    ...
    if (argc == 5 || argc == 6)
    {
        ...
        res = os_snprintf(cmd, sizeof(cmd), "WPS_ER_CONFIG %s %s %s %s %s %s", argv[0], argv[1], ssid_hex, argv[3],
                          argv[4], key_hex);
    }
    else
    {
...
#ifdef CONFIG_WPS_NFC
static int wpa_cli_cmd_wps_er_nfc_config_token(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc != 2)
    {
        PRINTF(
            "Invalid WPS_ER_NFC_CONFIG_TOKEN command: need two "
            "arguments:\n"
...
static int wpa_cli_cmd_identity(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid IDENTITY command: needs two arguments "
...
static int wpa_cli_cmd_password(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid PASSWORD command: needs two arguments "
...
static int wpa_cli_cmd_new_password(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[256], *pos, *end;
    int i, ret;
    ...
    {
        PRINTF(
            "Invalid NEW_PASSWORD command: needs two arguments "
            "(network id and password)\n");
        return -1;
    }
...
static int wpa_cli_cmd_pin(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid PIN command: needs two arguments "
            "(network id and pin)\n");
        return -1;
    }
...
static int wpa_cli_cmd_otp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid OTP command: needs two arguments (network "
            "id and password)\n");
        return -1;
    }
...
static int wpa_cli_cmd_sim(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid SIM command: needs two arguments "
...
static int wpa_cli_cmd_psk_passphrase(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[256], *pos, *end;
    int i, ret;
    ...
    pos = cmd;
    ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "PSK_PASSPHRASE-%s:%s", argv[0], argv[1]);
    if (os_snprintf_error(end - pos, ret))
    {
...
static int wpa_cli_cmd_passphrase(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[256], *pos, *end;
    int i, ret;
    ...
    {
        PRINTF(
            "Invalid PASSPHRASE command: needs two arguments "
...
static int wpa_cli_cmd_bssid(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    {
        PRINTF(
            "Invalid BSSID command: needs two arguments (network "
            "id and BSSID)\n");
        return -1;
    }
...
static int wpa_cli_cmd_bssid_ignore(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_list_networks(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "LIST_NETWORKS", 0, argc, argv);
}

static int wpa_cli_cmd_select_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "SELECT_NETWORK", 1, argc, argv);
}

static int wpa_cli_cmd_enable_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "ENABLE_NETWORK", 1, argc, argv);
}

static int wpa_cli_cmd_disable_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_remove_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_get_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc == 0)
    {
        wpa_cli_show_network_variables();
...
static int wpa_cli_cmd_dup_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc == 0)
    {
        wpa_cli_show_network_variables();
...
static int wpa_cli_cmd_list_creds(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_remove_cred(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static const char *const cred_fields[] = {"temporary",
                                          "priority",
                                          "sp_priority",
                                          "pcsc",
                                          "eap",
...
static int wpa_cli_cmd_disconnect(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "DISCONNECT");
}

static int wpa_cli_cmd_reconnect(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "RECONNECT");
}

static int wpa_cli_cmd_save_config(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_abort_scan(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_get_capability(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc < 1 || argc > 3)
    {
    ...
    if (argc > 1 && os_strcmp(argv[0], "key_mgmt") != 0 && os_strncmp(argv[1], "iftype=", 7) == 0)
    {
        PRINTF("Invalid GET_CAPABILITY command: 'iftype=' param is allowed only for 'key_mgmt'\n");
        return -1;
    }

    if (argc == 2 && os_strcmp(argv[1], "strict") != 0 && os_strncmp(argv[1], "iftype=", 7) != 0)
    {
        PRINTF(
            "Invalid GET_CAPABILITY command: the second argument, if any, must be 'strict' OR "
            "'iftype=<iftype_name>'\n");
        return -1;
    }
...
static char **wpa_cli_complete_get_capability(const char *str, int pos)
{
    int arg              = get_cmd_arg_num(str, pos);
    const char *fields[] = {
        "eap",  "pairwise", "group", "group_mgmt", "key_mgmt", "proto", "auth_alg", "modes", "channels", "freq",
#ifdef CONFIG_TDLS
        "tdls",
#endif /* CONFIG_TDLS */
#ifdef CONFIG_ERP
    ...
    const char *iftypes[] = {
        "iftype=STATION", "iftype=AP",  "iftype=P2P_CLIENT", "iftype=P2P_GO", "iftype=AP_VLAN",
        "iftype=IBSS",    "iftype=NAN", "iftype=P2P_DEVICE", "iftype=MESH",
    };
    int i, num_fields = ARRAY_SIZE(fields);
...
static int wpa_cli_cmd_reconfigure(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "RECONFIGURE");
}

static int wpa_cli_cmd_terminate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "TERMINATE");
}

static int wpa_cli_cmd_interface_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[256];
    int res;
    ...
    {
        PRINTF(
            "Invalid INTERFACE_ADD command: needs at least one "
     ...
     */
    res =
        os_snprintf(cmd, sizeof(cmd), "INTERFACE_ADD %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", argv[0], argc > 1 ? argv[1] : "",
                    argc > 2 ? argv[2] : "", argc > 3 ? argv[3] : "", argc > 4 ? argv[4] : "", argc > 5 ? argv[5] : "",
                    argc > 6 ? argv[6] : "", argc > 7 ? argv[7] : "");
    if (os_snprintf_error(sizeof(cmd), res))
        return -1;
    cmd[sizeof(cmd) - 1] = '\0';
    return wpa_ctrl_command(ctrl, cmd);
...
static int wpa_cli_cmd_interface_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "INTERFACE_REMOVE", 1, argc, argv);
}

static int wpa_cli_cmd_interface_list(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_ctrl_command_sta(struct wpa_ctrl *ctrl, const char *cmd, char *addr, size_t addr_len, int print)
{
    char buf[4096], *pos;
    size_t len;
    int ret;
...
static int wpa_cli_cmd_list_sta(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char addr[32], cmd[64];
...
static int wpa_cli_cmd_deauthenticate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_disassociate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_chanswitch(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "CHAN_SWITCH", 2, argc, argv);
}

static int wpa_cli_cmd_update_beacon(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_mesh_interface_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_INTERFACE_ADD", 0, argc, argv);
}

static int wpa_cli_cmd_mesh_group_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_GROUP_ADD", 1, argc, argv);
}

static int wpa_cli_cmd_mesh_group_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_GROUP_REMOVE", 1, argc, argv);
}

static int wpa_cli_cmd_mesh_peer_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_PEER_REMOVE", 1, argc, argv);
}

static int wpa_cli_cmd_mesh_peer_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MESH_PEER_ADD", 1, argc, argv);
}

static int wpa_cli_cmd_mesh_link_probe(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_p2p_stop_find(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "P2P_STOP_FIND");
}

static int wpa_cli_cmd_p2p_asp_provision(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_ASP_PROVISION", 3, argc, argv);
}

static int wpa_cli_cmd_p2p_asp_provision_resp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_ASP_PROVISION_RESP", 2, argc, argv);
}

static int wpa_cli_cmd_p2p_connect(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_p2p_listen(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_LISTEN", 0, argc, argv);
}

static int wpa_cli_cmd_p2p_group_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_p2p_group_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_GROUP_ADD", 0, argc, argv);
}

static int wpa_cli_cmd_p2p_group_member(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_GROUP_MEMBER", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_prov_disc(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc != 2 && argc != 3)
    {
        PRINTF(
            "Invalid P2P_PROV_DISC command: needs at least "
...
static int wpa_cli_cmd_p2p_get_passphrase(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "P2P_GET_PASSPHRASE");
}

static int wpa_cli_cmd_p2p_serv_disc_req(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[4096];
    ...
    {
        PRINTF(
            "Invalid P2P_SERV_DISC_REQ command: needs two "
...
static int wpa_cli_cmd_p2p_serv_disc_cancel_req(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_SERV_DISC_CANCEL_REQ", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_serv_disc_resp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[4096];
    int res;
    ...
    {
        PRINTF(
            "Invalid P2P_SERV_DISC_RESP command: needs four "
            "arguments (freq, address, dialog token, and TLVs)\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "P2P_SERV_DISC_RESP %s %s %s %s", argv[0], argv[1], argv[2], argv[3]);
    if (os_snprintf_error(sizeof(cmd), res))
        return -1;
    cmd[sizeof(cmd) - 1] = '\0';
    return wpa_ctrl_command(ctrl, cmd);
...
static int wpa_cli_cmd_p2p_service_update(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "P2P_SERVICE_UPDATE");
}

static int wpa_cli_cmd_p2p_serv_disc_external(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_SERV_DISC_EXTERNAL", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_service_flush(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "P2P_SERVICE_FLUSH");
}

static int wpa_cli_cmd_p2p_service_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc < 3)
    {
...
static int wpa_cli_cmd_p2p_service_rep(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc < 5 || argc > 6)
    {
        PRINTF(
            "Invalid P2P_SERVICE_REP command: needs 5-6 "
            "arguments\n");
        return -1;
    }
...
static int wpa_cli_cmd_p2p_service_del(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[4096];
    int res;
    ...
    {
        PRINTF(
            "Invalid P2P_SERVICE_DEL command: needs two or three "
            "arguments\n");
        return -1;
    }

    if (argc == 3)
        res = os_snprintf(cmd, sizeof(cmd), "P2P_SERVICE_DEL %s %s %s", argv[0], argv[1], argv[2]);
    else
        res = os_snprintf(cmd, sizeof(cmd), "P2P_SERVICE_DEL %s %s", argv[0], argv[1]);
    if (os_snprintf_error(sizeof(cmd), res))
        return -1;
    cmd[sizeof(cmd) - 1] = '\0';
    return wpa_ctrl_command(ctrl, cmd);
...
static int wpa_cli_cmd_p2p_reject(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_REJECT", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_invite(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_ctrl_command_p2p_peer(
    struct wpa_ctrl *ctrl, const char *cmd, char *addr, size_t addr_len, int discovered)
{
    char buf[4096], *pos;
    size_t len;
    int ret;
...
static int wpa_cli_cmd_p2p_peers(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    if (wpa_ctrl_command_p2p_peer(ctrl, "P2P_PEER FIRST", addr, sizeof(addr), discovered))
        return -1;
    do
    {
        os_snprintf(cmd, sizeof(cmd), "P2P_PEER NEXT-%s", addr);
    } while (wpa_ctrl_command_p2p_peer(ctrl, cmd, addr, sizeof(addr), discovered) == 0);
...
static char **wpa_cli_complete_p2p_set(const char *str, int pos)
{
    int arg              = get_cmd_arg_num(str, pos);
    const char *fields[] = {
        "discoverability", "managed",  "listen_channel", "ssid_postfix",  "noa",           "ps",
        "oppps",           "ctwindow", "disabled",       "conc_pref",     "force_long_sd", "peer_filter",
        "cross_connect",   "go_apsd",  "client_apsd",    "disallow_freq", "disc_int",      "per_sta_psk",
    };
    int i, num_fields = ARRAY_SIZE(fields);
...
static int wpa_cli_cmd_p2p_cancel(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "P2P_CANCEL");
}

static int wpa_cli_cmd_p2p_unauthorize(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_UNAUTHORIZE", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_presence_req(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_p2p_ext_listen(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    if (argc != 0 && argc != 2)
    {
        PRINTF(
            "Invalid P2P_EXT_LISTEN command: needs two arguments "
...
static int wpa_cli_cmd_p2p_remove_client(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_vendor_elem_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "VENDOR_ELEM_ADD", 2, argc, argv);
}

static int wpa_cli_cmd_vendor_elem_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "VENDOR_ELEM_GET", 1, argc, argv);
}

static int wpa_cli_cmd_vendor_elem_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_wfd_subelem_set(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[100];
    int res;
    ...
    {
        PRINTF(
            "Invalid WFD_SUBELEM_SET command: needs one or two "
            "arguments (subelem, hexdump)\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "WFD_SUBELEM_SET %s %s", argv[0], argc > 1 ? argv[1] : "");
    if (os_snprintf_error(sizeof(cmd), res))
        return -1;
    cmd[sizeof(cmd) - 1] = '\0';
    return wpa_ctrl_command(ctrl, cmd);
...
static int wpa_cli_cmd_wfd_subelem_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[100];
    int res;
    ...
    {
        PRINTF(
            "Invalid WFD_SUBELEM_GET command: needs one "
            "argument (subelem)\n");
        return -1;
    }
...
static int wpa_cli_cmd_fetch_anqp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "FETCH_ANQP");
}

static int wpa_cli_cmd_stop_fetch_anqp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "STOP_FETCH_ANQP");
}

static int wpa_cli_cmd_interworking_select(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "INTERWORKING_SELECT", 0, argc, argv);
}

static int wpa_cli_cmd_interworking_connect(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "INTERWORKING_CONNECT", 1, argc, argv);
}

static int wpa_cli_cmd_interworking_add_network(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_gas_request(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "GAS_REQUEST", 2, argc, argv);
}

static int wpa_cli_cmd_gas_response_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_hs20_anqp_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "HS20_ANQP_GET", 2, argc, argv);
}

static int wpa_cli_cmd_get_nai_home_realm_list(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[512];
    ...
    {
        PRINTF(
            "Command needs one or two arguments (dst mac addr and "
            "optional home realm)\n");
        return -1;
    }

    if (write_cmd(cmd, sizeof(cmd), "HS20_GET_NAI_HOME_REALM_LIST", argc, argv) < 0)
        return -1;

    return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_hs20_icon_request(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    char cmd[512];
    ...
    {
        PRINTF(
            "Command needs two arguments (dst mac addr and "
            "icon name)\n");
        return -1;
    }
...
static int wpa_cli_cmd_cancel_fetch_osu(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_sta_autoconnect(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "STA_AUTOCONNECT", 1, argc, argv);
}

static int wpa_cli_cmd_tdls_discover(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_DISCOVER", 1, argc, argv);
}

static int wpa_cli_cmd_tdls_setup(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_SETUP", 1, argc, argv);
}

static int wpa_cli_cmd_tdls_teardown(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_TEARDOWN", 1, argc, argv);
}

static int wpa_cli_cmd_tdls_link_status(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_LINK_STATUS", 1, argc, argv);
}

static int wpa_cli_cmd_wmm_ac_addts(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WMM_AC_ADDTS", 3, argc, argv);
}

static int wpa_cli_cmd_wmm_ac_delts(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "WMM_AC_DELTS", 1, argc, argv);
}

static int wpa_cli_cmd_wmm_ac_status(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "WMM_AC_STATUS");
}

static int wpa_cli_cmd_tdls_chan_switch(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_CHAN_SWITCH", 2, argc, argv);
}

static int wpa_cli_cmd_tdls_cancel_chan_switch(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TDLS_CANCEL_CHAN_SWITCH", 1, argc, argv);
}

static int wpa_cli_cmd_signal_poll(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "SIGNAL_POLL");
}

static int wpa_cli_cmd_signal_monitor(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "SIGNAL_MONITOR", 0, argc, argv);
}

static int wpa_cli_cmd_pktcnt_poll(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "PKTCNT_POLL");
}

static int wpa_cli_cmd_reauthenticate(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_neighbor_rep_request(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "NEIGHBOR_REP_REQUEST", 0, argc, argv);
}

static int wpa_cli_cmd_twt_setup(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "TWT_SETUP", 0, argc, argv);
}

static int wpa_cli_cmd_twt_teardown(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_mac_rand_scan(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "MAC_RAND_SCAN", 1, argc, argv);
}

static int wpa_cli_cmd_get_pref_freq_list(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "GET_PREF_FREQ_LIST", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_lo_start(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_LO_START", 4, argc, argv);
}

static int wpa_cli_cmd_p2p_lo_stop(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_dpp_qr_code(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_QR_CODE", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_bootstrap_gen(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_BOOTSTRAP_GEN", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_bootstrap_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_BOOTSTRAP_REMOVE", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_bootstrap_get_uri(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_BOOTSTRAP_GET_URI", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_bootstrap_info(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_BOOTSTRAP_INFO", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_bootstrap_set(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_BOOTSTRAP_SET", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_auth_init(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_AUTH_INIT", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_listen(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_LISTEN", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_stop_listen(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "DPP_STOP_LISTEN");
}

static int wpa_cli_cmd_dpp_configurator_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CONFIGURATOR_ADD", 0, argc, argv);
}

static int wpa_cli_cmd_dpp_configurator_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CONFIGURATOR_REMOVE", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_configurator_get_key(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CONFIGURATOR_GET_KEY", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_configurator_sign(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CONFIGURATOR_SIGN", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_pkex_add(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_PKEX_ADD", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_pkex_remove(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_dpp_controller_start(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CONTROLLER_START", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_controller_stop(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_ctrl_command(ctrl, "DPP_CONTROLLER_STOP");
}

static int wpa_cli_cmd_dpp_chirp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "DPP_CHIRP", 1, argc, argv);
}

static int wpa_cli_cmd_dpp_stop_chirp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
{
    char buf[512], *pos, *bssid = NULL, *freq = NULL, *level = NULL, *flags = NULL, *ssid = NULL;
    size_t len;
    int ret, id = -1;
    ...
    if (id != -1)
        PRINTF("%s\t%s\t%s\t%s\t%s\n", bssid ? bssid : "N/A", freq ? freq : "N/A", level ? level : "N/A",
...
static int wpa_cli_cmd_all_bss(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    ...
    mask = WPA_BSS_MASK_ID | WPA_BSS_MASK_BSSID | WPA_BSS_MASK_FREQ | WPA_BSS_MASK_LEVEL | WPA_BSS_MASK_FLAGS |
           WPA_BSS_MASK_SSID;
    do
    {
        if (id < 0)
            os_snprintf(cmd, sizeof(cmd), "BSS FIRST MASK=0x%x", mask);
        else
            os_snprintf(cmd, sizeof(cmd), "BSS NEXT-%d MASK=0x%x", id, mask);
...
static int wpa_cli_cmd_pasn_auth_start(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "PASN_AUTH_START", 4, argc, argv);
}

static int wpa_cli_cmd_pasn_auth_stop(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "PASN_AUTH_STOP", 0, argc, argv);
}

static int wpa_cli_cmd_ptksa_cache_list(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
    return wpa_cli_cmd(ctrl, "PTKSA_CACHE_LIST", 0, argc, argv);
}

static int wpa_cli_cmd_pasn_deauth(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
    {"status", wpa_cli_cmd_status, NULL, cli_cmd_flag_none, "[verbose] = get current WPA/EAPOL/EAP status"},
    {"ifname", wpa_cli_cmd_ifname, NULL, cli_cmd_flag_none, "= get current interface name"},
    {"ping", wpa_cli_cmd_ping, NULL, cli_cmd_flag_none, "= pings wpa_supplicant"},
    {"relog", wpa_cli_cmd_relog, NULL, cli_cmd_flag_none, "= re-open log-file (allow rolling logs)"},
    {"note", wpa_cli_cmd_note, NULL, cli_cmd_flag_none, "<text> = add a note to wpa_supplicant debug log"},
    {"mib", wpa_cli_cmd_mib, NULL, cli_cmd_flag_none, "= get MIB variables (dot1x, dot11)"},
#ifdef notyet
    {"help", wpa_cli_cmd_help, wpa_cli_complete_help, cli_cmd_flag_none, "[command] = show usage help"},
    {"interface", wpa_cli_cmd_interface, NULL, cli_cmd_flag_none, "[ifname] = show interfaces/select interface"},
    {"level", wpa_cli_cmd_level, NULL, cli_cmd_flag_none, "<debug level> = change debug level"},
    {"license", wpa_cli_cmd_license, NULL, cli_cmd_flag_none, "= show full wpa_cli license"},
    ...
    {"set", wpa_cli_cmd_set, wpa_cli_complete_set, cli_cmd_flag_none,
     ...
     "arguments)"},
    {"dump", wpa_cli_cmd_dump, NULL, cli_cmd_flag_none, "= dump config variables"},
    {"get", wpa_cli_cmd_get, wpa_cli_complete_get, cli_cmd_flag_none, "<name> = get information"},
    {"driver_flags", wpa_cli_cmd_driver_flags, NULL, cli_cmd_flag_none, "= list driver flags"},
...
#ifdef CONFIG_PMKSA_CACHE_EXTERNAL
    {"pmksa_get", wpa_cli_cmd_pmksa_get, NULL, cli_cmd_flag_none,
     "<network_id> = fetch all stored PMKSA cache entries"},
    {"pmksa_add", wpa_cli_cmd_pmksa_add, NULL, cli_cmd_flag_sensitive,
     "<network_id> <BSSID> <PMKID> <PMK> <reauth_time in seconds> <expiration in seconds> <akmp> <opportunistic> = "
     "store PMKSA cache entry from external storage"},
#ifdef CONFIG_MESH
...
#endif /* CONFIG_MESH */
#endif /* CONFIG_PMKSA_CACHE_EXTERNAL */
    {"reassociate", wpa_cli_cmd_reassociate, NULL, cli_cmd_flag_none, "= force reassociation"},
    {"reattach", wpa_cli_cmd_reattach, NULL, cli_cmd_flag_none, "= force reassociation back to the same BSS"},
    {"preauthenticate", wpa_cli_cmd_preauthenticate, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<BSSID> = force preauthentication"},
    {"identity", wpa_cli_cmd_identity, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> <identity> = configure identity for an SSID"},
    {"password", wpa_cli_cmd_password, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <password> = configure password for an SSID"},
    {"new_password", wpa_cli_cmd_new_password, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <password> = change password for an SSID"},
    {"pin", wpa_cli_cmd_pin, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <pin> = configure pin for an SSID"},
    {"otp", wpa_cli_cmd_otp, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <password> = configure one-time-password for an SSID"},
    {"psk_passphrase", wpa_cli_cmd_psk_passphrase, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <PSK/passphrase> = configure PSK/passphrase for an SSID"},
    {"passphrase", wpa_cli_cmd_passphrase, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <passphrase> = configure private key passphrase\n"
     "  for an SSID"},
    {"sim", wpa_cli_cmd_sim, wpa_cli_complete_network_id, cli_cmd_flag_sensitive,
     "<network id> <pin> = report SIM operation result"},
    {"bssid", wpa_cli_cmd_bssid, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> <BSSID> = set preferred BSSID for an SSID"},
    {"bssid_ignore", wpa_cli_cmd_bssid_ignore, wpa_cli_complete_bss, cli_cmd_flag_none,
    ...
    {"blacklist", /* deprecated alias for bssid_ignore */
     wpa_cli_cmd_bssid_ignore, wpa_cli_complete_bss, cli_cmd_flag_none, "= deprecated alias for bssid_ignore"},
    ...
    {"select_network", wpa_cli_cmd_select_network, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> = select a network (disable others)"},
    {"enable_network", wpa_cli_cmd_enable_network, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> = enable a network"},
    {"disable_network", wpa_cli_cmd_disable_network, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> = disable a network"},
    {"add_network", wpa_cli_cmd_add_network, NULL, cli_cmd_flag_none, "= add a network"},
    {"remove_network", wpa_cli_cmd_remove_network, wpa_cli_complete_network_id, cli_cmd_flag_none,
     "<network id> = remove a network"},
    {"set_network", wpa_cli_cmd_set_network, wpa_cli_complete_network, cli_cmd_flag_sensitive,
     "<network id> <variable> <value> = set network variables (shows\n"
     "  list of variables when run without arguments)"},
    {"get_network", wpa_cli_cmd_get_network, wpa_cli_complete_network, cli_cmd_flag_none,
     "<network id> <variable> = get network variables"},
    {"dup_network", wpa_cli_cmd_dup_network, wpa_cli_complete_dup_network, cli_cmd_flag_none,
    ...
    {"set_cred", wpa_cli_cmd_set_cred, wpa_cli_complete_cred, cli_cmd_flag_sensitive,
    ...
    {"save_config", wpa_cli_cmd_save_config, NULL, cli_cmd_flag_none, "= save the current configuration"},
    ...
    {"abort_scan", wpa_cli_cmd_abort_scan, NULL, cli_cmd_flag_none, "= request ongoing scan to be aborted"},
    {"bss", wpa_cli_cmd_bss, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<<idx> | <bssid>> = get detailed scan result info"},
    {"get_capability", wpa_cli_cmd_get_capability, wpa_cli_complete_get_capability, cli_cmd_flag_none,
    ...
    {"terminate", wpa_cli_cmd_terminate, NULL, cli_cmd_flag_none, "= terminate wpa_supplicant"},
    {"interface_add", wpa_cli_cmd_interface_add, NULL, cli_cmd_flag_none,
     ...
     "AP ('ap')"},
    {"interface_remove", wpa_cli_cmd_interface_remove, NULL, cli_cmd_flag_none, "<ifname> = removes the interface"},
    {"interface_list", wpa_cli_cmd_interface_list, NULL, cli_cmd_flag_none, "= list available interfaces"},
    {"ap_scan", wpa_cli_cmd_ap_scan, NULL, cli_cmd_flag_none, "<value> = set ap_scan parameter"},
    ...
    {"wps_pin", wpa_cli_cmd_wps_pin, wpa_cli_complete_bss, cli_cmd_flag_sensitive,
     "<BSSID> [PIN] = start WPS PIN method (returns PIN, if not "
     "hardcoded)"},
    {"wps_check_pin", wpa_cli_cmd_wps_check_pin, NULL, cli_cmd_flag_sensitive, "<PIN> = verify PIN checksum"},
    {"wps_cancel", wpa_cli_cmd_wps_cancel, NULL, cli_cmd_flag_none, "Cancels the pending WPS operation"},
#ifdef CONFIG_WPS_NFC
    {"wps_nfc", wpa_cli_cmd_wps_nfc, wpa_cli_complete_bss, cli_cmd_flag_none,
     "[BSSID] = start Wi-Fi Protected Setup: NFC"},
    {"wps_nfc_config_token", wpa_cli_cmd_wps_nfc_config_token, NULL, cli_cmd_flag_none,
    ...
    {"wps_nfc_tag_read", wpa_cli_cmd_wps_nfc_tag_read, NULL, cli_cmd_flag_sensitive,
     "<hexdump of payload> = report read NFC tag with WPS data"},
    {"nfc_get_handover_req", wpa_cli_cmd_nfc_get_handover_req, NULL, cli_cmd_flag_none,
     "<NDEF> <WPS> = create NFC handover request"},
    {"nfc_get_handover_sel", wpa_cli_cmd_nfc_get_handover_sel, NULL, cli_cmd_flag_none,
     "<NDEF> <WPS> = create NFC handover select"},
    {"nfc_report_handover", wpa_cli_cmd_nfc_report_handover, NULL, cli_cmd_flag_none,
     "<role> <type> <hexdump of req> <hexdump of sel> = report completed "
     "NFC handover"},
#endif /* CONFIG_WPS_NFC */
    {"wps_reg", wpa_cli_cmd_wps_reg, wpa_cli_complete_bss, cli_cmd_flag_sensitive,
    ...
    {"wps_er_set_config", wpa_cli_cmd_wps_er_set_config, NULL, cli_cmd_flag_none,
...
#ifdef CONFIG_WPS_NFC
    {"wps_er_nfc_config_token", wpa_cli_cmd_wps_er_nfc_config_token, NULL, cli_cmd_flag_none,
     "<WPS/NDEF> <UUID> = build NFC configuration token"},
#endif /* CONFIG_WPS_NFC */
    {"ibss_rsn", wpa_cli_cmd_ibss_rsn, NULL, cli_cmd_flag_none,
     "<addr> = request RSN authentication with <addr> in IBSS"},
#ifdef CONFIG_AP
    {"sta", wpa_cli_cmd_sta, wpa_cli_complete_sta, cli_cmd_flag_none,
     "<addr> = get information about an associated station (AP)"},
    {"all_sta", wpa_cli_cmd_all_sta, NULL, cli_cmd_flag_none, "= get information about all associated stations (AP)"},
    {"list_sta", wpa_cli_cmd_list_sta, NULL, cli_cmd_flag_none, "= list all stations (AP)"},
    {"deauthenticate", wpa_cli_cmd_deauthenticate, wpa_cli_complete_deauthenticate, cli_cmd_flag_none,
     "<addr> = deauthenticate a station"},
    {"disassociate", wpa_cli_cmd_disassociate, wpa_cli_complete_disassociate, cli_cmd_flag_none,
     "<addr> = disassociate a station"},
    {"chan_switch", wpa_cli_cmd_chanswitch, NULL, cli_cmd_flag_none,
     "<cs_count> <freq> [sec_channel_offset=] [center_freq1=]"
     " [center_freq2=] [bandwidth=] [blocktx] [ht|vht]"
     " = CSA parameters"},
    {"update_beacon", wpa_cli_cmd_update_beacon, NULL, cli_cmd_flag_none, "= update Beacon frame contents"},
#endif /* CONFIG_AP */
    {"suspend", wpa_cli_cmd_suspend, NULL, cli_cmd_flag_none, "= notification of suspend/hibernate"},
    {"resume", wpa_cli_cmd_resume, NULL, cli_cmd_flag_none, "= notification of resume/thaw"},
#ifdef CONFIG_TESTING_OPTIONS
    {"drop_sa", wpa_cli_cmd_drop_sa, NULL, cli_cmd_flag_none, "= drop SA without deauth/disassoc (test command)"},
#endif /* CONFIG_TESTING_OPTIONS */
    {"roam", wpa_cli_cmd_roam, wpa_cli_complete_bss, cli_cmd_flag_none, "<addr> = roam to the specified BSS"},
#ifdef CONFIG_MESH
    {"mesh_interface_add", wpa_cli_cmd_mesh_interface_add, NULL, cli_cmd_flag_none,
    ...
    {"mesh_group_remove", wpa_cli_cmd_mesh_group_remove, NULL, cli_cmd_flag_none,
...
#endif /* CONFIG_MESH */
#ifdef CONFIG_P2P
    {"p2p_find", wpa_cli_cmd_p2p_find, wpa_cli_complete_p2p_find, cli_cmd_flag_none,
     "[timeout] [type=*] = find P2P Devices for up-to timeout seconds"},
    {"p2p_stop_find", wpa_cli_cmd_p2p_stop_find, NULL, cli_cmd_flag_none, "= stop P2P Devices search"},
    {"p2p_asp_provision", wpa_cli_cmd_p2p_asp_provision, NULL, cli_cmd_flag_none,
     "<addr> adv_id=<adv_id> conncap=<conncap> [info=<infodata>] = provision with a P2P ASP Device"},
    {"p2p_asp_provision_resp", wpa_cli_cmd_p2p_asp_provision_resp, NULL, cli_cmd_flag_none,
     "<addr> adv_id=<adv_id> [role<conncap>] [info=<infodata>] = provision with a P2P ASP Device"},
    {"p2p_connect", wpa_cli_cmd_p2p_connect, wpa_cli_complete_p2p_connect, cli_cmd_flag_none,
    ...
    {"p2p_group_remove", wpa_cli_cmd_p2p_group_remove, wpa_cli_complete_p2p_group_remove, cli_cmd_flag_none,
    ...
    {"p2p_prov_disc", wpa_cli_cmd_p2p_prov_disc, wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
     "<addr> <method> = request provisioning discovery"},
    {"p2p_get_passphrase", wpa_cli_cmd_p2p_get_passphrase, NULL, cli_cmd_flag_none,
     "= get the passphrase for a group (GO only)"},
    {"p2p_serv_disc_req", wpa_cli_cmd_p2p_serv_disc_req, wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
     "<addr> <TLVs> = schedule service discovery request"},
    {"p2p_serv_disc_cancel_req", wpa_cli_cmd_p2p_serv_disc_cancel_req, NULL, cli_cmd_flag_none,
     "<id> = cancel pending service discovery request"},
    {"p2p_serv_disc_resp", wpa_cli_cmd_p2p_serv_disc_resp, NULL, cli_cmd_flag_none,
     "<freq> <addr> <dialog token> <TLVs> = service discovery response"},
    {"p2p_service_update", wpa_cli_cmd_p2p_service_update, NULL, cli_cmd_flag_none,
     "= indicate change in local services"},
    {"p2p_serv_disc_external", wpa_cli_cmd_p2p_serv_disc_external, NULL, cli_cmd_flag_none,
     "<external> = set external processing of service discovery"},
    {"p2p_service_flush", wpa_cli_cmd_p2p_service_flush, NULL, cli_cmd_flag_none,
    ...
    {"p2p_service_del", wpa_cli_cmd_p2p_service_del, NULL, cli_cmd_flag_none,
     ...
     "service"},
    {"p2p_reject", wpa_cli_cmd_p2p_reject, wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
    ...
    {"p2p_set", wpa_cli_cmd_p2p_set, wpa_cli_complete_p2p_set, cli_cmd_flag_none,
    ...
    {"p2p_unauthorize", wpa_cli_cmd_p2p_unauthorize, wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
    ...
    {"p2p_remove_client", wpa_cli_cmd_p2p_remove_client, wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
    ...
    {"vendor_elem_remove", wpa_cli_cmd_vendor_elem_remove, NULL, cli_cmd_flag_none,
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_INTERWORKING
    {"fetch_anqp", wpa_cli_cmd_fetch_anqp, NULL, cli_cmd_flag_none, "= fetch ANQP information for all APs"},
    {"stop_fetch_anqp", wpa_cli_cmd_stop_fetch_anqp, NULL, cli_cmd_flag_none, "= stop fetch_anqp operation"},
    {"interworking_select", wpa_cli_cmd_interworking_select, NULL, cli_cmd_flag_none,
     "[auto] = perform Interworking network selection"},
    {"interworking_connect", wpa_cli_cmd_interworking_connect, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<BSSID> = connect using Interworking credentials"},
    {"interworking_add_network", wpa_cli_cmd_interworking_add_network, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<BSSID> = connect using Interworking credentials"},
    {"anqp_get", wpa_cli_cmd_anqp_get, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> <info id>[,<info id>]... = request ANQP information"},
    {"gas_request", wpa_cli_cmd_gas_request, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> <AdvProtoID> [QueryReq] = GAS request"},
    {"gas_response_get", wpa_cli_cmd_gas_response_get, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> <dialog token> [start,len] = Fetch last GAS response"},
#endif /* CONFIG_INTERWORKING */
#ifdef CONFIG_HS20
    {"hs20_anqp_get", wpa_cli_cmd_hs20_anqp_get, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> <subtype>[,<subtype>]... = request HS 2.0 ANQP information"},
    {"nai_home_realm_list", wpa_cli_cmd_get_nai_home_realm_list, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> <home realm> = get HS20 nai home realm list"},
    {"hs20_icon_request", wpa_cli_cmd_hs20_icon_request, wpa_cli_complete_bss, cli_cmd_flag_none,
    ...
    {"tdls_setup", wpa_cli_cmd_tdls_setup, NULL, cli_cmd_flag_none, "<addr> = request TDLS setup with <addr>"},
    {"tdls_teardown", wpa_cli_cmd_tdls_teardown, NULL, cli_cmd_flag_none, "<addr> = tear down TDLS with <addr>"},
    ...
    {"tdls_chan_switch", wpa_cli_cmd_tdls_chan_switch, NULL, cli_cmd_flag_none,
     ...
     "with TDLS peer"},
    {"tdls_cancel_chan_switch", wpa_cli_cmd_tdls_cancel_chan_switch, NULL, cli_cmd_flag_none,
     "<addr> = disable channel switching with TDLS peer <addr>"},
    {"signal_poll", wpa_cli_cmd_signal_poll, NULL, cli_cmd_flag_none, "= get signal parameters"},
    {"signal_monitor", wpa_cli_cmd_signal_monitor, NULL, cli_cmd_flag_none, "= set signal monitor parameters"},
    {"pktcnt_poll", wpa_cli_cmd_pktcnt_poll, NULL, cli_cmd_flag_none, "= get TX/RX packet counters"},
    ...
    {"wnm_bss_query", wpa_cli_cmd_wnm_bss_query, NULL, cli_cmd_flag_none,
     "<query reason> [list]"
     " [neighbor=<BSSID>,<BSSID information>,<operating class>,<channel number>,<PHY type>[,<hexdump of optional "
     "subelements>]"
     " = Send BSS Transition Management Query"},
#endif /* CONFIG_WNM */
    {"raw", wpa_cli_cmd_raw, NULL, cli_cmd_flag_sensitive, "<params..> = Sent unprocessed command"},
    {"flush", wpa_cli_cmd_flush, NULL, cli_cmd_flag_none, "= flush wpa_supplicant state"},
#ifdef ANDROID
    {"driver", wpa_cli_cmd_driver, NULL, cli_cmd_flag_none, "<command> = driver private commands"},
#endif /* ANDROID */
    {"radio_work", wpa_cli_cmd_radio_work, NULL, cli_cmd_flag_none, "= radio_work <show/add/done>"},
    {"vendor", wpa_cli_cmd_vendor, NULL, cli_cmd_flag_none,
     "<vendor id> <command id> [<hex formatted command argument>] = Send vendor command"},
    {"neighbor_rep_request", wpa_cli_cmd_neighbor_rep_request, NULL, cli_cmd_flag_none,
     "[ssid=<SSID>] [lci] [civic] = Trigger request to AP for neighboring AP report (with optional given SSID in hex "
     "or enclosed in double quotes, default: current SSID; with optional LCI and location civic request)"},
    {"twt_setup", wpa_cli_cmd_twt_setup, NULL, cli_cmd_flag_none,
     "[dialog=<token>] [exponent=<exponent>] [mantissa=<mantissa>] [min_twt=<Min TWT>] [setup_cmd=<setup-cmd>] "
     "[twt=<u64>] [requestor=0|1] [trigger=0|1] [implicit=0|1] [flow_type=0|1] [flow_id=<3-bit-id>] [protection=0|1] "
     "[twt_channel=<twt chanel id>] [control=<control-u8>] = Send TWT Setup frame"},
    {"twt_teardown", wpa_cli_cmd_twt_teardown, NULL, cli_cmd_flag_none, "[flags=<value>] = Send TWT Teardown frame"},
    {"erp_flush", wpa_cli_cmd_erp_flush, NULL, cli_cmd_flag_none, "= flush ERP keys"},
    ...
    {"get_pref_freq_list", wpa_cli_cmd_get_pref_freq_list, NULL, cli_cmd_flag_none,
    ...
    {"p2p_lo_stop", wpa_cli_cmd_p2p_lo_stop, NULL, cli_cmd_flag_none, "= stop P2P listen offload"},
#ifdef CONFIG_DPP
    {"dpp_qr_code", wpa_cli_cmd_dpp_qr_code, NULL, cli_cmd_flag_none, "report a scanned DPP URI from a QR Code"},
    {"dpp_bootstrap_gen", wpa_cli_cmd_dpp_bootstrap_gen, NULL, cli_cmd_flag_sensitive,
     "type=<qrcode> [chan=..] [mac=..] [info=..] [curve=..] [key=..] = generate DPP bootstrap information"},
    {"dpp_bootstrap_remove", wpa_cli_cmd_dpp_bootstrap_remove, NULL, cli_cmd_flag_none,
     "*|<id> = remove DPP bootstrap information"},
    {"dpp_bootstrap_get_uri", wpa_cli_cmd_dpp_bootstrap_get_uri, NULL, cli_cmd_flag_none,
     "<id> = get DPP bootstrap URI"},
    {"dpp_bootstrap_info", wpa_cli_cmd_dpp_bootstrap_info, NULL, cli_cmd_flag_none,
     "<id> = show DPP bootstrap information"},
    {"dpp_bootstrap_set", wpa_cli_cmd_dpp_bootstrap_set, NULL, cli_cmd_flag_none,
     "<id> [conf=..] [ssid=<SSID>] [ssid_charset=#] [psk=<PSK>] [pass=<passphrase>] [configurator=<id>] "
     "[conn_status=#] [akm_use_selector=<0|1>] [group_id=..] [expiry=#] [csrattrs=..] = set DPP configurator "
     "parameters"},
    ...
    {"dpp_configurator_add", wpa_cli_cmd_dpp_configurator_add, NULL, cli_cmd_flag_sensitive,
     "[curve=..] [key=..] = add DPP configurator"},
    {"dpp_configurator_remove", wpa_cli_cmd_dpp_configurator_remove, NULL, cli_cmd_flag_none,
     "*|<id> = remove DPP configurator"},
    {"dpp_configurator_get_key", wpa_cli_cmd_dpp_configurator_get_key, NULL, cli_cmd_flag_none,
     "<id> = Get DPP configurator's private key"},
    {"dpp_configurator_sign", wpa_cli_cmd_dpp_configurator_sign, NULL, cli_cmd_flag_none,
...
#ifdef CONFIG_DPP2
    {"dpp_controller_start", wpa_cli_cmd_dpp_controller_start, NULL, cli_cmd_flag_none,
     "[tcp_port=<port>] [role=..] = start DPP controller"},
    {"dpp_controller_stop", wpa_cli_cmd_dpp_controller_stop, NULL, cli_cmd_flag_none, "= stop DPP controller"},
    {"dpp_chirp", wpa_cli_cmd_dpp_chirp, NULL, cli_cmd_flag_none, "own=<BI ID> iter=<count> = start DPP chirp"},
    {"dpp_stop_chirp", wpa_cli_cmd_dpp_stop_chirp, NULL, cli_cmd_flag_none, "= stop DPP chirp"},
#endif /* CONFIG_DPP2 */
#endif /* CONFIG_DPP */
    {"all_bss", wpa_cli_cmd_all_bss, NULL, cli_cmd_flag_none, "= list all BSS entries (scan results)"},
#ifdef CONFIG_PASN
    {"pasn_auth_start", wpa_cli_cmd_pasn_auth_start, NULL, cli_cmd_flag_none,
     "bssid=<BSSID> akmp=<WPA key mgmt> cipher=<WPA cipher> group=<group> nid=<network id> = Start PASN "
     "authentication"},
    {"pasn_auth_stop", wpa_cli_cmd_pasn_auth_stop, NULL, cli_cmd_flag_none, "= Stop PASN authentication"},
    {"ptksa_cache_list", wpa_cli_cmd_ptksa_cache_list, NULL, cli_cmd_flag_none, "= Get the PTKSA Cache"},
    {"pasn_deauth", wpa_cli_cmd_pasn_deauth, NULL, cli_cmd_flag_none, "bssid=<BSSID> = Remove PASN PTKSA state"},
#endif /* CONFIG_PASN */
    {"mscs", wpa_cli_cmd_mscs, NULL, cli_cmd_flag_none,
     "<add|remove|change> [up_bitmap=<hex byte>] [up_limit=<integer>] [stream_timeout=<in TUs>] [frame_classifier=<hex "
     "bytes>] = Configure MSCS request"},
    {"scs", wpa_cli_cmd_scs, NULL, cli_cmd_flag_none,
     "[scs_id=<decimal number>] <add|remove|change> [scs_up=<0-7>] [classifier_type=<4|10>] [classifier params based "
     "on classifier type] [tclas_processing=<0|1>] [scs_id=<decimal number>] ... = Send SCS request"},
    {"dscp_resp", wpa_cli_cmd_dscp_resp, NULL, cli_cmd_flag_none,
     "<[reset]>/<[solicited] [policy_id=1 status=0...]> [more] = Send DSCP response"},
    {"dscp_query", wpa_cli_cmd_dscp_query, NULL, cli_cmd_flag_none, "wildcard/domain_name=<string> = Send DSCP Query"},
...
#ifdef INTERACTIVE
static char **wpa_cli_cmd_completion(const char *cmd, const char *str, int pos)
{
    int i;
...
#ifdef INTERACTIVE
static int wpa_cli_exec(const char *program, const char *arg1, const char *arg2)
{
    char *arg;
    size_t len;
    int res;
...
{
    wpa_cli_close_connection();
    if ((global && wpa_cli_open_global_ctrl() < 0) || (!global && wpa_cli_open_connection(ctrl_ifname, 1) < 0))
        return;
...
static int check_terminating(const char *msg)
{
    ...
    if (str_starts(pos, WPA_EVENT_TERMINATING) && ctrl_conn)
    {
        edit_clear_line();
        PRINTF(
            "\rConnection to wpa_supplicant lost - trying to "
            "reconnect\n");
        edit_redraw();
        wpa_cli_attached = 0;
        wpa_cli_close_connection();
...
static void wpa_cli_recv_pending(struct wpa_ctrl *ctrl, int action_monitor)
{
    ...
    {
        PRINTF(
            "Connection to wpa_supplicant lost - trying to "
            "reconnect\n");
        if (reconnect)
        {
            eloop_terminate();
...
static void start_edit(void)
{
    ...
    if (edit_init(wpa_cli_edit_cmd_cb, wpa_cli_edit_eof_cb, wpa_cli_edit_completion_cb, NULL, hfile, ps) < 0)
    {
        eloop_terminate();
        return;
    }
...
static void wpa_cli_action_ping(void *eloop_ctx, void *timeout_ctx)
{
    ...
    len = sizeof(buf) - 1;
    if (wpa_ctrl_request(ctrl, "PING", 4, buf, &len, wpa_cli_action_cb) < 0 || len < 4 ||
        os_memcmp(buf, "PONG", 4) != 0)
    {
...
int cli_main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        c = cli_getopt(argc, argv, "a:Bg:G:hi:p:P:rs:v");
        if (c < 0)
            break;
        switch (c)
        {
            case 'a':
                action_file = cli_optarg;
                break;
            case 'B':
                daemonize = 1;
                break;
            ...
            case 'i':
                os_free(ctrl_ifname);
                ctrl_ifname = os_strdup(cli_optarg);
                break;
            case 'p':
                ctrl_iface_dir = cli_optarg;
                break;
            case 'P':
                pid_file = cli_optarg;
                break;
...
#ifdef INTERACTIVE
    interactive = (argc == cli_optind) && (action_file == NULL);
