...
static int wpa_config_validate_network(struct wpa_ssid *ssid, int line)
{
    ...
    if ((ssid->group_cipher & WPA_CIPHER_CCMP) &&
        !(ssid->pairwise_cipher &
          (WPA_CIPHER_CCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP | WPA_CIPHER_GCMP_256 | WPA_CIPHER_NONE)))
    {
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: PMF needs to be enabled whenever using OCV", line);
        errors++;
    }
#endif /* CONFIG_OCV */
...
static struct wpa_ssid *wpa_config_read_network(FILE *f, int *line, int id)
{
    ...
    while (wpa_config_get_line(buf, sizeof(buf), f, line, &pos))
    {
        ...
        *pos2++ = '\0';
        ...
        {
#ifndef CONFIG_WEP
            if (os_strcmp(pos, "wep_key0") == 0 || os_strcmp(pos, "wep_key1") == 0 || os_strcmp(pos, "wep_key2") == 0 ||
                os_strcmp(pos, "wep_key3") == 0 || os_strcmp(pos, "wep_tx_keyidx") == 0)
            {
                wpa_printf(MSG_ERROR, "Line %d: unsupported WEP parameter", *line);
                ssid->disabled = 1;
                continue;
            }
#endif /* CONFIG_WEP */
...
static struct wpa_config_blob *wpa_config_read_blob(FILE *f, int *line, const char *name)
{
    ...
    blob->data = wpa_base64_decode(encoded, encoded_len, &blob->len);
    os_free(encoded);
    ...
    if (cred->num_roaming_consortiums)
    {
        ...
        for (i = 0; i < cred->num_roaming_consortiums; i++)
        {
            ...
            for (j = 0; j < cred->roaming_consortiums_len[i]; j++)
                fprintf(f, "%02x", cred->roaming_consortiums[i][j]);
        }
        fprintf(f, "\"\n");
    }
...
static int wpa_config_write_blob(FILE *f, struct wpa_config_blob *blob)
{
    ...
    encoded = wpa_base64_encode(blob->data, blob->len, NULL);
    if (encoded == NULL)
        return -1;
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
...
#ifdef CONFIG_P2P
    {
        ...
        {
            buf = wps_dev_type_bin2str(config->sec_device_type[i], _buf, sizeof(_buf));
            if (buf)
    ...
    if (WPA_GET_BE32(config->ip_addr_go))
        fprintf(f, "ip_addr_go=%u.%u.%u.%u\n", config->ip_addr_go[0], config->ip_addr_go[1], config->ip_addr_go[2],
                config->ip_addr_go[3]);
    ...
    if (WPA_GET_BE32(config->ip_addr_end))
        fprintf(f, "ip_addr_end=%u.%u.%u.%u\n", config->ip_addr_end[0], config->ip_addr_end[1], config->ip_addr_end[2],
    ...
    if (config->initial_freq_list && config->initial_freq_list[0])
    {
        ...
        {
            fprintf(f, "%s%d", i > 0 ? " " : "", config->initial_freq_list[i]);
        }
        fprintf(f, "\n");
    }
    ...
    if (config->scan_res_valid_for_connect != DEFAULT_SCAN_RES_VALID_FOR_CONNECT)
    ...
    if (config->dot11RSNASAERetransPeriod != DEFAULT_DOT11_RSNA_SAE_RETRANS_PERIOD)
    ...
    if (config->disassoc_imminent_rssi_threshold != DEFAULT_DISASSOC_IMMINENT_RSSI_THRESHOLD)
...
int wpa_config_write(const char *name, struct wpa_config *config)
{
    ...
    for (ssid = config->ssid; ssid; ssid = ssid->next)
    {
        ...
        if (wpa_key_mgmt_wpa_psk_no_sae(ssid->key_mgmt) && !ssid->psk_set && !ssid->passphrase)
            continue; /* do not save invalid network */
        if (wpa_key_mgmt_sae(ssid->key_mgmt) && !ssid->passphrase && !ssid->sae_password)
            continue; /* do not save invalid network */
        fprintf(f, "\nnetwork={\n");
