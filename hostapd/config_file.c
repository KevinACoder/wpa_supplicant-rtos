...
static int hostapd_config_read_vlan_file(struct hostapd_bss_config *bss, const char *fname)
{
    FILE *f;
...
static int hostapd_config_read_maclist(const char *fname, struct mac_acl_entry **acl, int *num)
{
    FILE *f;
    char buf[128], *pos;
    int line = 0;
...
static int hostapd_config_eap_user_salted(
    struct hostapd_eap_user *user, const char *hash, size_t len, char **pos, int line, const char *fname)
{
    char *pos2 = *pos;
    ...
    if (pos2 - *pos < (int)(2 * (len + 1)))
    { /* at least 1 byte of salt */
        wpa_printf(MSG_ERROR, "Invalid salted %s hash on line %d in '%s'", hash, line, fname);
        return -1;
    }
    ...
    if (!user->password)
    {
        wpa_printf(MSG_ERROR, "Failed to allocate memory for salted %s hash", hash);
        return -1;
    }

    if (hexstr2bin(*pos, user->password, len) < 0)
    {
        wpa_printf(MSG_ERROR, "Invalid salted password on line %d in '%s'", line, fname);
        return -1;
    }
    ...
    *pos += 2 * len;
    ...
    {
        wpa_printf(MSG_ERROR, "Invalid salt for password on line %d in '%s'", line, fname);
        return -1;
    }
...
static int hostapd_config_read_eap_user(const char *fname, struct hostapd_bss_config *conf)
{
    FILE *f;
    ...
    if (os_strncmp(fname, "sqlite:", 7) == 0)
    {
        ...
        return 0;
#else  /* CONFIG_SQLITE */
        wpa_printf(MSG_ERROR, "EAP user file in SQLite DB, but CONFIG_SQLITE was not enabled in the build.");
        return -1;
#endif /* CONFIG_SQLITE */
    }
    ...
    while (fgets(buf, sizeof(buf), f))
    {
        ...
        if (user && os_strncmp(buf, "radius_accept_attr=", 19) == 0)
        {
            ...
            if (attr == NULL)
            {
                wpa_printf(MSG_ERROR, "Invalid radius_accept_attr: %s", buf + 19);
        ...
        {
            pos += 6;
            if (hostapd_config_eap_user_salted(user, "sha1", 20, &pos, line, fname) < 0)
                goto failed;
        }
        else if (os_strncmp(pos, "ssha256:", 8) == 0)
        {
            pos += 8;
            if (hostapd_config_eap_user_salted(user, "sha256", 32, &pos, line, fname) < 0)
                goto failed;
        }
        else if (os_strncmp(pos, "ssha512:", 8) == 0)
        {
            pos += 8;
            if (hostapd_config_eap_user_salted(user, "sha512", 64, &pos, line, fname) < 0)
                goto failed;
        }
        else
        {
...
#ifndef CONFIG_NO_RADIUS
static int hostapd_config_read_radius_addr(struct hostapd_radius_server **server,
                                           int *num_server,
                                           const char *val,
                                           int def_port,
...
static int hostapd_config_bss(struct hostapd_config *conf, const char *ifname)
{
    ...
    all = os_realloc_array(conf->bss, conf->num_bss + 1, sizeof(struct hostapd_bss_config *));
    if (all == NULL)
    {
        wpa_printf(MSG_ERROR,
...
static int rkh_derive_key(const char *pos, u8 *key, size_t key_len)
{
    ...
    ret = hmac_sha256_kdf(oldkey, sizeof(oldkey), "FT OLDKEY", NULL, 0, key, key_len);
...
static int hostapd_config_ht_capab(struct hostapd_config *conf, const char *capab)
{
...
static int hostapd_config_vht_capab(struct hostapd_config *conf, const char *capab)
{
    ...
    if (os_strstr(capab, "[BF-ANTENNA-2]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMEE_CAPABLE))
        conf->vht_capab |= (1 << VHT_CAP_BEAMFORMEE_STS_OFFSET);
    if (os_strstr(capab, "[BF-ANTENNA-3]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMEE_CAPABLE))
        conf->vht_capab |= (2 << VHT_CAP_BEAMFORMEE_STS_OFFSET);
    if (os_strstr(capab, "[BF-ANTENNA-4]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMEE_CAPABLE))
        conf->vht_capab |= (3 << VHT_CAP_BEAMFORMEE_STS_OFFSET);
    if (os_strstr(capab, "[SOUNDING-DIMENSION-2]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMER_CAPABLE))
        conf->vht_capab |= (1 << VHT_CAP_SOUNDING_DIMENSION_OFFSET);
    if (os_strstr(capab, "[SOUNDING-DIMENSION-3]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMER_CAPABLE))
        conf->vht_capab |= (2 << VHT_CAP_SOUNDING_DIMENSION_OFFSET);
    if (os_strstr(capab, "[SOUNDING-DIMENSION-4]") && (conf->vht_capab & VHT_CAP_SU_BEAMFORMER_CAPABLE))
    ...
    if (os_strstr(capab, "[VHT-LINK-ADAPT2]") && (conf->vht_capab & VHT_CAP_HTC_VHT))
        conf->vht_capab |= VHT_CAP_VHT_LINK_ADAPTATION_VHT_UNSOL_MFB;
    if (os_strstr(capab, "[VHT-LINK-ADAPT3]") && (conf->vht_capab & VHT_CAP_HTC_VHT))
...
static int parse_roaming_consortium(struct hostapd_bss_config *bss, char *pos, int line)
{
    ...
    if ((len & 1) || len < 2 * 3 || len / 2 > MAX_ROAMING_CONSORTIUM_LEN || hexstr2bin(pos, oi, len / 2))
    {
        wpa_printf(MSG_ERROR,
    ...
    rc = os_realloc_array(bss->roaming_consortium, bss->roaming_consortium_count + 1,
...
static int parse_lang_string(struct hostapd_lang_string **array, unsigned int *count, char *pos)
{
    char *sep, *str = NULL;
...
static int parse_venue_name(struct hostapd_bss_config *bss, char *pos, int line)
{
...
static int parse_venue_url(struct hostapd_bss_config *bss, char *pos, int line)
{
    char *sep;
    size_t nlen;
    ...
    *sep++ = '\0';
    ...
    url = os_realloc_array(bss->venue_url, bss->venue_url_count + 1, sizeof(struct hostapd_venue_url));
    if (!url)
        goto fail;
...
fail:
    if (ret)
        wpa_printf(MSG_ERROR, "Line %d: Invalid venue_url '%s'", line, pos);
    return ret;
}

static int parse_3gpp_cell_net(struct hostapd_bss_config *bss, char *buf, int line)
{
    size_t count;
    char *pos;
    u8 *info = NULL, *ipos;
    ...
    *ipos++ = count; /* Number of PLMNs */
    ...
    while (pos && *pos)
    {
        ...
        *ipos++ = (((mnc_len == 2) ? 0xf0 : ((mnc[2] - '0') << 4))) | (mcc[2] - '0');
    ...
    wpa_hexdump(MSG_MSGDUMP, "3GPP Cellular Network information", bss->anqp_3gpp_cell_net, bss->anqp_3gpp_cell_net_len);
...
static int parse_nai_realm(struct hostapd_bss_config *bss, char *buf, int line)
{
    ...
    for (i = 0; i < bss->nai_realm_count; i++)
    {
        ...
        {
            offsets[i * MAX_NAI_REALMS + j] = realm->realm[j] ? realm->realm[j] - realm->realm_buf : -1;
        }
    }

    realm = os_realloc_array(bss->nai_realm_data, bss->nai_realm_count + 1, sizeof(struct hostapd_nai_realm_data));
    if (realm == NULL)
    {
        os_free(offsets);
        return -1;
...
static int parse_qos_map_set(struct hostapd_bss_config *bss, char *buf, int line)
{
...
#ifdef CONFIG_HS20
static int hs20_parse_conn_capab(struct hostapd_bss_config *bss, char *buf, int line)
{
    u8 *conn_cap;
    char *pos;
    ...
    conn_cap = os_realloc(bss->hs20_connection_capability, bss->hs20_connection_capability_len + 4);
    if (conn_cap == NULL)
        return -1;
...
static int hs20_parse_wan_metrics(struct hostapd_bss_config *bss, char *buf, int line)
{
    u8 *wan_metrics;
    char *pos;
...
static int hs20_parse_oper_friendly_name(struct hostapd_bss_config *bss, char *pos, int line)
{
    if (parse_lang_string(&bss->hs20_oper_friendly_name, &bss->hs20_oper_friendly_name_count, pos))
    {
        wpa_printf(MSG_ERROR,
                   "Line %d: Invalid "
...
static int hs20_parse_icon(struct hostapd_bss_config *bss, char *pos)
{
    ...
    icon = os_realloc_array(bss->hs20_icons, bss->hs20_icons_count + 1, sizeof(struct hs20_icon));
    if (icon == NULL)
        return -1;
    bss->hs20_icons = icon;
...
static int hs20_parse_osu_ssid(struct hostapd_bss_config *bss, char *pos, int line)
{
    size_t slen;
    char *str;
...
static int hs20_parse_osu_server_uri(struct hostapd_bss_config *bss, char *pos, int line)
{
    struct hs20_osu_provider *p;

    p = os_realloc_array(bss->hs20_osu_providers, bss->hs20_osu_providers_count + 1, sizeof(*p));
    if (p == NULL)
        return -1;
...
static int hs20_parse_osu_friendly_name(struct hostapd_bss_config *bss, char *pos, int line)
{
    ...
    if (parse_lang_string(&bss->last_osu->friendly_name, &bss->last_osu->friendly_name_count, pos))
    {
...
static int hs20_parse_osu_nai(struct hostapd_bss_config *bss, char *pos, int line)
{
    if (bss->last_osu == NULL)
    {
        wpa_printf(MSG_ERROR, "Line %d: Unexpected OSU field", line);
...
static int hs20_parse_osu_nai2(struct hostapd_bss_config *bss, char *pos, int line)
{
    if (bss->last_osu == NULL)
    {
        wpa_printf(MSG_ERROR, "Line %d: Unexpected OSU field", line);
...
static int hs20_parse_osu_method_list(struct hostapd_bss_config *bss, char *pos, int line)
{
    if (bss->last_osu == NULL)
    {
        wpa_printf(MSG_ERROR, "Line %d: Unexpected OSU field", line);
...
static int hs20_parse_osu_icon(struct hostapd_bss_config *bss, char *pos, int line)
{
    char **n;
...
static int hs20_parse_osu_service_desc(struct hostapd_bss_config *bss, char *pos, int line)
{
    if (bss->last_osu == NULL)
    {
        wpa_printf(MSG_ERROR, "Line %d: Unexpected OSU field", line);
    ...
    if (parse_lang_string(&bss->last_osu->service_desc, &bss->last_osu->service_desc_count, pos))
    {
...
static int hs20_parse_operator_icon(struct hostapd_bss_config *bss, char *pos, int line)
{
    char **n;

    n = os_realloc_array(bss->hs20_operator_icon, bss->hs20_operator_icon_count + 1, sizeof(char *));
    if (!n)
        return -1;
...
#ifdef CONFIG_ACS
static int hostapd_config_parse_acs_chan_bias(struct hostapd_config *conf, char *pos)
{
...
#ifdef CONFIG_DPP2
static int hostapd_dpp_controller_parse(struct hostapd_bss_config *bss, const char *pos)
{
    ...
    if (!val || os_strlen(val) != 2 * SHA256_MAC_LEN || hexstr2bin(val, conf->pkhash, SHA256_MAC_LEN) < 0)
        goto fail;
    os_free(val);
...
static int get_hex_config(u8 *buf, size_t max_len, int line, const char *field, const char *val)
{
...
static int hostapd_config_fill(
    struct hostapd_config *conf, struct hostapd_bss_config *bss, const char *buf, char *pos, int line)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Line %d: DEPRECATED: 'debug' configuration variable is not used anymore", line);
    }
    ...
    {
        wpa_printf(MSG_INFO, "Line %d: DEPRECATED: 'dump_file' configuration variable is not used anymore", line);
    }
    ...
    else if (os_strcmp(buf, "macaddr_acl") == 0)
    {
        ...
        if (acl != ACCEPT_UNLESS_DENIED && acl != DENY_UNLESS_ACCEPTED && acl != USE_EXTERNAL_RADIUS_AUTH)
        {
            wpa_printf(MSG_ERROR, "Line %d: unknown macaddr_acl %d", line, acl);
            return 1;
        }
        bss->macaddr_acl = acl;
    }
    ...
    {
        if (hostapd_config_read_maclist(pos, &bss->accept_mac, &bss->num_accept_mac))
        {
            wpa_printf(MSG_ERROR, "Line %d: Failed to read accept_mac_file '%s'", line, pos);
            return 1;
        }
    }
    else if (os_strcmp(buf, "deny_mac_file") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Failed to read deny_mac_file '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "country_code") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid country_code '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "eapol_version") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid EAPOL version (%d): '%s'.", line, eapol_version, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "check_cert_subject") == 0)
    {
        if (!pos[0])
        {
            wpa_printf(MSG_ERROR, "Line %d: unknown check_cert_subject '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "eap_fast_a_id") == 0)
    {
        ...
        if (bss->eap_fast_a_id == NULL || hexstr2bin(pos, bss->eap_fast_a_id, idlen / 2))
        {
    ...
    else if (os_strcmp(buf, "eap_teap_auth") == 0)
    {
        ...
        if (val < 0 || val > 2)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid eap_teap_auth value", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "eap_message") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Failed to allocate memory for eap_req_id_text", line);
            return 1;
        }
        ...
        {
            *term++ = '\0';
            os_memmove(term, term + 1, bss->eap_req_id_text_len - (term - bss->eap_req_id_text) - 1);
    ...
    else if (os_strcmp(buf, "wep_key_len_broadcast") == 0)
    {
        ...
        if (val < 0 || val > 13)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid WEP key len %d (= %d bits)", line, val, val * 8);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wep_rekey_period") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid period %d", line, bss->wep_rekeying_period);
            return 1;
        }
#endif /* CONFIG_WEP */
    }
    else if (os_strcmp(buf, "eap_reauth_period") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid period %d", line, bss->eap_reauth_period);
            return 1;
        }
    }
    ...
    {
        if (hostapd_config_read_radius_addr(&bss->radius->auth_servers, &bss->radius->num_auth_servers, pos, 1812,
    ...
    else if (bss->radius->auth_server && os_strcmp(buf, "auth_server_addr_replace") == 0)
    {
    ...
    else if (bss->radius->auth_server && os_strcmp(buf, "auth_server_port") == 0)
    {
        bss->radius->auth_server->port = atoi(pos);
    }
    else if (bss->radius->auth_server && os_strcmp(buf, "auth_server_shared_secret") == 0)
    {
        int len = os_strlen(pos);
        if (len == 0)
        {
            /* RFC 2865, Ch. 3 */
            wpa_printf(MSG_ERROR, "Line %d: empty shared secret is not allowed", line);
            return 1;
        }
    ...
    {
        if (hostapd_config_read_radius_addr(&bss->radius->acct_servers, &bss->radius->num_acct_servers, pos, 1813,
    ...
    else if (bss->radius->acct_server && os_strcmp(buf, "acct_server_addr_replace") == 0)
    {
    ...
    else if (bss->radius->acct_server && os_strcmp(buf, "acct_server_port") == 0)
    {
        bss->radius->acct_server->port = atoi(pos);
    }
    else if (bss->radius->acct_server && os_strcmp(buf, "acct_server_shared_secret") == 0)
    {
        int len = os_strlen(pos);
        if (len == 0)
        {
    ...
    else if (os_strcmp(buf, "auth_algs") == 0)
    {
        ...
        if (bss->auth_algs == 0)
        {
            wpa_printf(MSG_ERROR, "Line %d: no authentication algorithms allowed", line);
            return 1;
        }
    }
    else if (os_strcmp(buf, "max_num_sta") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid max_num_sta=%d; allowed range 0..%d", line, bss->max_num_sta,
                       MAX_STA_COUNT);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "extended_key_id") == 0)
    {
        ...
        if (val < 0 || val > 2)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid extended_key_id=%d; allowed range 0..2", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wpa_deny_ptk0_rekey") == 0)
    {
        ...
        if (bss->wpa_deny_ptk0_rekey < 0 || bss->wpa_deny_ptk0_rekey > 2)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid wpa_deny_ptk0_rekey=%d; allowed range 0..2", line,
                       bss->wpa_deny_ptk0_rekey);
            return 1;
        }
    }
    else if (os_strcmp(buf, "wpa_group_update_count") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid wpa_group_update_count=%lu; allowed range 1..4294967295", line,
                       val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wpa_pairwise_update_count") == 0)
    {
        ...
        if (*endp || val < 1 || val > (u32)-1)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid wpa_pairwise_update_count=%lu; allowed range 1..4294967295", line,
                       val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wpa_passphrase") == 0)
    {
        ...
        if (len < 8 || len > 63)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid WPA passphrase length %d (expected 8..63)", line, len);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wpa_psk") == 0)
    {
        ...
        if (bss->ssid.wpa_psk == NULL)
            return 1;
        if (hexstr2bin(pos, bss->ssid.wpa_psk->psk, PMK_LEN) || pos[PMK_LEN * 2] != '\0')
        {
    ...
    else if (os_strcmp(buf, "wpa_psk_radius") == 0)
    {
        ...
        if (bss->wpa_psk_radius != PSK_RADIUS_IGNORED && bss->wpa_psk_radius != PSK_RADIUS_ACCEPTED &&
            bss->wpa_psk_radius != PSK_RADIUS_REQUIRED)
        {
            wpa_printf(MSG_ERROR, "Line %d: unknown wpa_psk_radius %d", line, bss->wpa_psk_radius);
            return 1;
        }
    }
    else if (os_strcmp(buf, "wpa_pairwise") == 0)
    {
        ...
        if (bss->wpa_pairwise == -1 || bss->wpa_pairwise == 0)
            return 1;
        if (bss->wpa_pairwise & (WPA_CIPHER_NONE | WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104))
        {
            wpa_printf(MSG_ERROR, "Line %d: unsupported pairwise cipher suite '%s'", line, pos);
            return 1;
        }
    }
    else if (os_strcmp(buf, "rsn_pairwise") == 0)
    {
        ...
        if (bss->rsn_pairwise == -1 || bss->rsn_pairwise == 0)
            return 1;
        if (bss->rsn_pairwise & (WPA_CIPHER_NONE | WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104))
        {
            wpa_printf(MSG_ERROR, "Line %d: unsupported pairwise cipher suite '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "group_cipher") == 0)
    {
        ...
        if (bss->group_cipher == -1 || bss->group_cipher == 0)
            return 1;
        if (bss->group_cipher != WPA_CIPHER_TKIP && bss->group_cipher != WPA_CIPHER_CCMP &&
            bss->group_cipher != WPA_CIPHER_GCMP && bss->group_cipher != WPA_CIPHER_GCMP_256 &&
            bss->group_cipher != WPA_CIPHER_CCMP_256)
        {
            wpa_printf(MSG_ERROR, "Line %d: unsupported group cipher suite '%s'", line, pos);
            return 1;
        }
    ...
    {
        wpa_printf(MSG_INFO, "Line %d: Obsolete peerkey parameter ignored", line);
#ifdef CONFIG_IEEE80211R_AP
    }
    ...
    {
        if (os_strlen(pos) != 2 * FT_R1KH_ID_LEN || hexstr2bin(pos, bss->r1_key_holder, FT_R1KH_ID_LEN) != 0)
        {
    ...
    else if (os_strcmp(buf, "ctrl_interface_group") == 0)
    {
        ...
        if (grp)
        {
            ...
            wpa_printf(MSG_DEBUG, "ctrl_interface_group=%d (from group name '%s')", bss->ctrl_interface_gid, group);
            return 0;
        }
    ...
    else if (os_strcmp(buf, "channel") == 0)
    {
        ...
        {
#ifndef CONFIG_ACS
            wpa_printf(MSG_ERROR, "Line %d: tries to enable ACS but CONFIG_ACS disabled", line);
            return 1;
#else  /* CONFIG_ACS */
            conf->acs     = 1;
            conf->channel = 0;
    ...
    else if (os_strcmp(buf, "freqlist") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid frequency list", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "min_tx_power") == 0)
    {
        ...
        if (val < 0 || val > 255)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid min_tx_power %d (expected 0..255)", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "beacon_int") == 0)
    {
        ...
        if (val < 10 || val > 65535)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid beacon_int %d (expected 10..65535)", line, val);
            return 1;
        }
        conf->beacon_int = val;
#ifdef CONFIG_ACS
    ...
    else if (os_strcmp(buf, "acs_num_scans") == 0)
    {
        ...
        if (val <= 0 || val > 100)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid acs_num_scans %d (expected 1..100)", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "dtim_period") == 0)
    {
        ...
        if (val < 1 || val > 255)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid dtim_period %d", line, val);
            return 1;
        }
        bss->dtim_period = val;
    }
    else if (os_strcmp(buf, "bss_load_update_period") == 0)
    {
        ...
        if (val < 0 || val > 100)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid bss_load_update_period %d", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "chan_util_avg_period") == 0)
    {
        ...
        if (val < 0)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid chan_util_avg_period", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "rts_threshold") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid rts_threshold %d", line, conf->rts_threshold);
            return 1;
        }
    }
    else if (os_strcmp(buf, "fragm_threshold") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid fragm_threshold %d", line, conf->fragm_threshold);
            return 1;
        }
    }
    else if (os_strcmp(buf, "send_probe_response") == 0)
    {
        ...
        if (val != 0 && val != 1)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid send_probe_response %d (expected 0 or 1)", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "beacon_rate") == 0)
    {
        ...
        if (os_strncmp(pos, "ht:", 3) == 0)
        {
            ...
            if (val < 0 || val > 31)
            {
                wpa_printf(MSG_ERROR, "Line %d: invalid beacon_rate HT-MCS %d", line, val);
                return 1;
            }
        ...
        else if (os_strncmp(pos, "vht:", 4) == 0)
        {
            ...
            if (val < 0 || val > 9)
            {
                wpa_printf(MSG_ERROR, "Line %d: invalid beacon_rate VHT-MCS %d", line, val);
                return 1;
            }
        ...
        else if (os_strncmp(pos, "he:", 3) == 0)
        {
            ...
            if (val < 0 || val > 11)
            {
                wpa_printf(MSG_ERROR, "Line %d: invalid beacon_rate HE-MCS %d", line, val);
                return 1;
            }
        ...
        else
        {
            ...
            if (val < 10 || val > 10000)
            {
                wpa_printf(MSG_ERROR, "Line %d: invalid legacy beacon_rate %d", line, val);
                return 1;
            }
    ...
    else if (os_strcmp(buf, "wep_default_key") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Invalid wep_default_key index %d", bss->ssid.wep.idx);
            return 1;
        }
    }
    else if (os_strcmp(buf, "wep_key0") == 0 || os_strcmp(buf, "wep_key1") == 0 || os_strcmp(buf, "wep_key2") == 0 ||
    ...
    else if (os_strcmp(buf, "vlan_file") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: failed to read VLAN file '%s'", line, pos);
            return 1;
        }
    }
    else if (os_strcmp(buf, "vlan_naming") == 0)
    {
        ...
        if (bss->ssid.vlan_naming >= DYNAMIC_VLAN_NAMING_END || bss->ssid.vlan_naming < 0)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid naming scheme %d", line, bss->ssid.vlan_naming);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "wme_enabled") == 0 || os_strcmp(buf, "wmm_enabled") == 0)
    {
    ...
    else if (os_strncmp(buf, "wme_ac_", 7) == 0 || os_strncmp(buf, "wmm_ac_", 7) == 0)
    {
    ...
    else if (os_strcmp(buf, "group_mgmt_cipher") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid group_mgmt_cipher: %s", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "assoc_sa_query_max_timeout") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid assoc_sa_query_max_timeout", line);
            return 1;
        }
    }
    else if (os_strcmp(buf, "assoc_sa_query_retry_timeout") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid assoc_sa_query_retry_timeout", line);
            return 1;
        }
#ifdef CONFIG_OCV
    }
    ...
    {
        conf->he_mu_edca.he_qos_info |= set_he_cap(atoi(pos), HE_QOS_INFO_EDCA_PARAM_SET_COUNT);
    }
    else if (os_strcmp(buf, "he_mu_edca_qos_info_q_ack") == 0)
    {
        conf->he_mu_edca.he_qos_info |= set_he_cap(atoi(pos), HE_QOS_INFO_Q_ACK);
    }
    else if (os_strcmp(buf, "he_mu_edca_qos_info_queue_request") == 0)
    {
        conf->he_mu_edca.he_qos_info |= set_he_cap(atoi(pos), HE_QOS_INFO_QUEUE_REQUEST);
    }
    else if (os_strcmp(buf, "he_mu_edca_qos_info_txop_request") == 0)
    {
        conf->he_mu_edca.he_qos_info |= set_he_cap(atoi(pos), HE_QOS_INFO_TXOP_REQUEST);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_be_aifsn") == 0)
    {
        conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_AIFSN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_be_acm") == 0)
    {
        conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACM);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_be_aci") == 0)
    {
        conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACI);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_be_ecwmin") == 0)
    {
        conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMIN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_be_ecwmax") == 0)
    {
        conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMAX);
    }
    ...
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_AIFSN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_bk_acm") == 0)
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACM);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_bk_aci") == 0)
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACI);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_bk_ecwmin") == 0)
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMIN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_bk_ecwmax") == 0)
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMAX);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_bk_timer") == 0)
    {
        conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_TIMER_IDX] = atoi(pos) & 0xff;
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_aifsn") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_AIFSN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_acm") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACM);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_aci") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACI);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_ecwmin") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMIN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_ecwmax") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMAX);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vi_timer") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_TIMER_IDX] = atoi(pos) & 0xff;
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_aifsn") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_AIFSN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_acm") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACM);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_aci") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ACI_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ACI);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_ecwmin") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMIN);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_ecwmax") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ECW_IDX] |= set_he_cap(atoi(pos), HE_MU_AC_PARAM_ECWMAX);
    }
    else if (os_strcmp(buf, "he_mu_edca_ac_vo_timer") == 0)
    {
        conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_TIMER_IDX] = atoi(pos) & 0xff;
    }
    ...
    {
        if (hostapd_parse_he_srg_bitmap(conf->spr.srg_bss_color_bitmap, pos))
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid srg bss colors list '%s'", line, pos);
            return 1;
        }
    }
    else if (os_strcmp(buf, "he_spr_srg_partial_bssid") == 0)
    {
        if (hostapd_parse_he_srg_bitmap(conf->spr.srg_partial_bssid_bitmap, pos))
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid srg partial bssid list '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "unsol_bcast_probe_resp_interval") == 0)
    {
        ...
        if (val < 0 || val > 20)
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid unsol_bcast_probe_resp_interval value", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "extra_cred") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: could not read Credentials from '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "ap_settings") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: could not read AP Settings from '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "multi_ap_backhaul_wpa_psk") == 0)
    {
        ...
        if (!bss->multi_ap_backhaul_ssid.wpa_psk)
            return 1;
        if (hexstr2bin(pos, bss->multi_ap_backhaul_ssid.wpa_psk->psk, PMK_LEN) || pos[PMK_LEN * 2] != '\0')
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid PSK '%s'.", line, pos);
            hostapd_config_clear_wpa_psk(&bss->multi_ap_backhaul_ssid.wpa_psk);
            return 1;
    ...
    else if (os_strcmp(buf, "network_auth_type") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid network_auth_type '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "ipaddr_type_availability") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid ipaddr_type_availability '%s'", line, pos);
    ...
    else if (os_strcmp(buf, "gas_frag_limit") == 0)
    {
        ...
        if (val <= 0)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid gas_frag_limit '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "hs20_release") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Unsupported hs20_release: %s", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "hs20_operating_class") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid hs20_operating_class '%s'", line, pos);
            return 1;
        }
        oper_class_len /= 2;
...
#define PARSE_TEST_PROBABILITY(_val)                                         \
    ...
    else if (os_strcmp(buf, "sae_password") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid sae_password", line);
            return 1;
        }
#endif /* CONFIG_SAE */
    }
    ...
    else if (os_strcmp(buf, "sae_anti_clogging_threshold") == 0 || os_strcmp(buf, "anti_clogging_threshold") == 0)
    {
    ...
    else if (os_strcmp(buf, "sae_groups") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid sae_groups value '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "local_pwr_constraint") == 0)
    {
        ...
        if (val < 0 || val > 255)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid local_pwr_constraint %d (expected 0..255)", line, val);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "fst_group_id") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid fst_group_id value '%s'", line, pos);
            return 1;
        }

        if (conf->fst_cfg.group_id[0])
        {
            wpa_printf(MSG_ERROR, "Line %d: Duplicate fst_group value '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "fst_priority") == 0)
    {
        ...
        if (!*pos)
        {
            wpa_printf(MSG_ERROR, "Line %d: fst_priority value not supplied (expected 1..%u)", line,
                       FST_MAX_PRIO_VALUE);
            return -1;
        }
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid fst_priority %ld (%s) (expected 1..%u)", line, val, pos,
                       FST_MAX_PRIO_VALUE);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "fst_llt") == 0)
    {
        ...
        if (!*pos)
        {
            wpa_printf(MSG_ERROR, "Line %d: fst_llt value not supplied (expected 1..%u)", line, FST_MAX_LLT_MS);
            return -1;
        }
        val = strtol(pos, &endp, 0);
        if (*endp || val < 1 || (unsigned long int)val > FST_MAX_LLT_MS)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid fst_llt %ld (%s) (expected 1..%u)", line, val, pos, FST_MAX_LLT_MS);
            return 1;
        }
    ...
    {
        if (atoi(pos))
            bss->radio_measurements[0] |= WLAN_RRM_CAPS_BEACON_REPORT_PASSIVE | WLAN_RRM_CAPS_BEACON_REPORT_ACTIVE |
    ...
    else if (os_strcmp(buf, "fils_cache_id") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid fils_cache_id '%s'", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "dpp_pfs") == 0)
    {
        ...
        if (val < 0 || val > 2)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid dpp_pfs value '%s'", line, pos);
            return -1;
        }
        bss->dpp_pfs = val;
#endif /* CONFIG_DPP2 */
    ...
    else if (os_strcmp(buf, "owe_transition_bssid") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid owe_transition_bssid", line);
            return 1;
        }
    }
    ...
    {
        os_strlcpy(bss->owe_transition_ifname, pos, sizeof(bss->owe_transition_ifname));
    }
    else if (os_strcmp(buf, "owe_groups") == 0)
    {
        if (hostapd_parse_intlist(&bss->owe_groups, pos))
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid owe_groups value '%s'", line, pos);
            return 1;
        }
    }
    ...
    else if (os_strcmp(buf, "multi_ap") == 0)
    {
        ...
        if (val < 0 || val > 3)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid multi_ap '%s'", line, buf);
            return -1;
        }
    ...
    else if (os_strcmp(buf, "airtime_mode") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Unknown airtime_mode", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "airtime_bss_limit") == 0)
    {
        ...
        if (val < 0 || val > 1)
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid airtime_bss_limit (must be 0 or 1)", line);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "airtime_sta_weight") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid airtime weight '%s'", line, pos);
            return 1;
        }
#endif /* CONFIG_AIRTIME_POLICY */
#ifdef CONFIG_MACSEC
    ...
    else if (os_strcmp(buf, "macsec_policy") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid macsec_policy (%d): '%s'.", line, macsec_policy, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "macsec_integ_only") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid macsec_integ_only (%d): '%s'.", line, macsec_integ_only, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "macsec_replay_protect") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid macsec_replay_protect (%d): '%s'.", line, macsec_replay_protect,
                       pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "macsec_port") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid macsec_port (%d): '%s'.", line, macsec_port, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "mka_priority") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid mka_priority (%d): '%s'.", line, mka_priority, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "macsec_csindex") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: invalid macsec_csindex (%d): '%s'.", line, macsec_csindex, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "mka_cak") == 0)
    {
        ...
        if (len > 2 * MACSEC_CAK_MAX_LEN || (len != 2 * 16 && len != 2 * 32) || hexstr2bin(pos, bss->mka_cak, len / 2))
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid MKA-CAK '%s'.", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "mka_ckn") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid MKA-CKN '%s'.", line, pos);
            return 1;
        }
    ...
    else if (os_strcmp(buf, "pasn_groups") == 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Line %d: Invalid pasn_groups value '%s'", line, pos);
            return 1;
        }
    }
    ...
    {
        if (get_hex_config(bss->ext_capa_mask, EXT_CAPA_MAX_LEN, line, "ext_capa_mask", pos))
            return 1;
    }
    else if (os_strcmp(buf, "ext_capa") == 0)
    {
        if (get_hex_config(bss->ext_capa, EXT_CAPA_MAX_LEN, line, "ext_capa", pos))
            return 1;
    }
